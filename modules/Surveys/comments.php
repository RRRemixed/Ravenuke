	...
	if ($mode == 'nested') {
		...
		while ($row = $db->sql_fetchrow($result)) {
			...
			if ($r_score >= $thold) {
				...
				if ((isset($userinfo['commentmax'])) && (strlen($r_comment) > $userinfo['commentmax'])) echo substr($r_comment, 0, $userinfo['commentmax']) . '<br /><br /><b><a href="modules.php?name=' . $module_name . '&amp;file=comments&amp;pollID=' . $r_pollID . '&amp;tid=' . $r_tid . '">' . _READREST . '</a></b>';
				elseif (strlen($r_comment) > $commentlimit) echo substr($r_comment, 0, $commentlimit) . '<br /><br /><b><a href="modules.php?name=' . $module_name . '&amp;file=comments&amp;pollID=' . $r_pollID . '&amp;tid=' . $r_tid . '">' . _READREST . '</a></b>';
				else echo $r_comment . '<br /><br />';
				if ($anonpost == 1 OR is_admin($admin) OR is_user($user)) {
					echo '<p><font color="' . $textcolor2 . '"> [ <a href="modules.php?name=' . $module_name . '&amp;file=comments&amp;op=Reply&amp;pid=' . $r_tid . '&amp;pollID=' . $r_pollID . '">' . _REPLY . '</a>';
				}
				modtwo($r_tid, $r_score, $r_reason);
				if (is_admin($admin)) {
					echo ' | <a href="' . $admin_file . '.php?op=RemovePollComment&amp;tid=' . $r_tid . '&amp;pollID=' . $r_pollID . '">' . _DELETE . '</a> ]</font></p>';
		...
		while ($row = $db->sql_fetchrow($result)) {
			...
			if ($r_score >= $thold) {
				...
				if (!eregi('[a-z0-9]', $r_subject)) $r_subject = '[' . _NOSUBJECT . ']';
				formatTimestamp($r_date);
				echo '<li><font color="' . $textcolor2 . '"><a href="modules.php?name=' . $module_name . '&amp;file=comments&amp;op=showreply&amp;tid=' . $r_tid . '&amp;pollID=' . $r_pollID . '&amp;pid=' . $r_pid . '#' . $r_tid . '">' . $r_subject . '</a> ' . _BY . ' ' . $r_name . ' ' . _ON . ' ' . $datetime . '</font></li>';
...
function DisplayTopic($pollID, $pid = 0, $tid = 0, $mode = 'thread', $order = 0, $thold = 0, $level = 0, $nokids = 0) {
	...
	while ($count_times < $num_tid) {
		...
		echo '</p></td></tr><tr><td>';
		if ((isset($userinfo['commentmax'])) AND (strlen($comment) > $userinfo['commentmax'])) echo substr($comment, 0, $userinfo['commentmax']) . '<br /><br /><b><a href="modules.php?name=' . $module_name . '&amp;file=comments&amp;pollID=' . $pollID . '&amp;tid=' . $tid . '">' . _READREST . '</a></b>';
		elseif (strlen($comment) > $commentlimit) echo substr($comment, 0, $commentlimit) . '<br /><br /><b><a href="modules.php?name=' . $module_name . '&amp;file=comments&amp;pollID=' . $pollID . '&amp;tid=' . $tid . '">' . _READREST . '</a></b>';
		else echo $comment;
...
function reply($pid, $pollID, $mode, $order, $thold) {
	...
	if ($anonpost == 0 AND !is_user($user)) {
		...
		title(_SURVEYCOM);
		OpenTable();
		echo '<center><p><b>' . $subject . '</b></p></center>';
		if (empty($comment)) {
