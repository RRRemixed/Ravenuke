...
function pollMain($pollID) {
	...
	$boxContent .= '<p><a href="modules.php?name=' . $module_name . '&amp;op=results&amp;pollID=' . $pollID . '"><b>' . _RESULTS . '</b></a><br /><a href="modules.php?name=' . $module_name . '"><b>' . _POLLS . '</b></a></p><p>';
	if ($pollcomm) {
		list($numcom) = $db->sql_fetchrow($db->sql_query('SELECT COUNT(*) FROM ' . $prefix . '_pollcomments WHERE pollID=\'' . $pollID . '\''));
		$boxContent .= _VOTES . ': <b>' . $sum . '</b><br /> ' . _PCOMMENTS . ' <b>' . $numcom . '</b>';
	} else {
