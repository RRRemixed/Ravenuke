...
if ($opi != 'ds') {
	...
	if (eregi('(Content-Type)|(MIME-Version)|(Content-Disposition)|(\\\\n)|(%0A)|(0x0A)|(\\\\r)|(0x0D)|(%0D)|(to:)|(cc:)|(bcc:)', $sender_name)) {
		$name_err = '<center><font class="option"><b><i>'._FBINVALIDNAME.'</i></b></font></center><br />';
		$sender_name = '';
		$send = $FB_NO;
	}
	...
	if (!check_email_address($sender_email)) {
		$email_err = '<center><font class="option"><b><i>'._FBRENTEREMAIL.'</i></b></font></center><br />';
		$sender_email = '';
		$send = $FB_NO;
	}
	if (empty($subject)) {
		$subject_err = '<center><font class="option"><b><i>'._FBENTERSUBJECT.'</i></b></font></center><br />';
		$send = $FB_NO;
	}
	if (eregi('(Content-Type)|(MIME-Version)|(Content-Disposition)|(\\\\n)|(%0A)|(0x0A)|(\\\\r)|(0x0D)|(%0D)|(to:)|(cc:)|(bcc:)', $subject)) {
		$subject_err = '<center><font class="option"><b><i>'._FBINVALIDSUBJECT.'</i></b></font></center><br />';
		$subject = '';
		$send = $FB_NO;
	}
	if (empty($message)) {
...
******************************************************/
	...
	if (!security_code_check($gfx_check, $modGFXChk[$module_name])) {
		$sec_err = '<center><font class="option"><b><i>'._SECCODEINCOR.'</i></b></font></center><br />';
		$send = $FB_NO;
	}
/*****[END]********************************************
 [ Base:    GFX Code                           v1.0.0 ]
...
function form_block() {
	...
	echo '<div align="center"><font class="title"><b>'."$sitename".':&nbsp;'._FEEDBACKTITLE.'</b></font>'."\n"
		.'<p><font class="content">'._FEEDBACKNOTE.'</font></p>'."\n"
 ...
 ******************************************************/
	echo '<p><input type="submit" name="submit" value="'._SEND.'" /></p>'."\n"
		.'</form>'."\n"
		.'<p><b>'._ALLFIELDSREQUIRED.'</b></p></div>'."\n";
}
