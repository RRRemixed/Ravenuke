...
function links() {
	...
	OpenTable();
	echo "<center><font class=\"content\">[ <a class=\"rn_csrf\" href=\"".$admin_file.".php?op=LinksCleanVotes\">" . _CLEANLINKSDB . "</a> | "
	...
	while($row = $db->sql_fetchrow($result)) {
		...
		if ($recordexist == 0) {
			...
			while($row3 = $db->sql_fetchrow($resulted2)) {
				...
				echo "<center><font class=\"option\"><b>Modify Editorial</b></font></center><br /><br />"
					...
					."" . _EDITORIALTITLE . ":<br /><input type=\"text\" name=\"editorialtitle\" value=\"$editorialtitle\" size=\"50\" maxlength=\"100\" /><br />"
					."" . _EDITORIALTEXT . ":<br /><textarea name=\"editorialtext\" cols=\"60\" rows=\"10\">$editorialtext</textarea><br />"
					."</select><input type=\"hidden\" name=\"op\" value=\"LinksModEditorial\" /><input type=\"submit\" value=\"" . _MODIFY . "\" /> [ <a class=\"rn_csrf\" href=\"".$admin_file.".php?op=LinksDelEditorial&amp;linkid=$lid\">" . _DELETE . "</a> ]";
			}
		}
		CloseTable();
		echo "<br />";
		...
		while($row4 = $db->sql_fetchrow($result4)) {
			...
			$timestamp = mktime(0, 0, 0, $date_array['1'], $date_array['2'], $date_array['0']);
				...
				echo "<tr><td valign=top bgcolor=$colorswitch>$ratinguser</td><td valign=top colspan=5 bgcolor=$colorswitch>$ratingcomments</td><td bgcolor=$colorswitch><center><b><a class=\"rn_csrf\" href=".$admin_file.".php?op=LinksDelComment&lid=$lid&rid=$ratingdbid>X</a></b></center></td><br /></tr>";
			$x++;
		...
		while($row7 = $db->sql_fetchrow($result7)) {
			...
			$timestamp = mktime(0, 0, 0, $date_array['1'], $date_array['2'], $date_array['0']);
			$formatted_date = date("F j, Y", $timestamp);
			echo "<td colspan=2 bgcolor=$colorswitch>$ratinghostname</td><td colspan=3 bgcolor=$colorswitch>$rating</td><td bgcolor=$colorswitch>$formatted_date  </font></b></td><td bgcolor=$colorswitch><center><b><a class=\"rn_csrf\" href=\"".$admin_file.".php?op=LinksDelVote&lid=$lid&rid=$ratingdbid\">X</a></b></center></td></tr><br />";
			$x++;
			if ($colorswitch=="$bgcolor1") $colorswitch="$bgcolor2";
			else $colorswitch="$bgcolor1";
		}
		...
		while($row8 = $db->sql_fetchrow($result8)) {
			...
			$timestamp = mktime(0, 0, 0, $date_array['1'], $date_array['2'], $date_array['0']);
			$formatted_date = date("F j, Y", $timestamp);
			echo "<tr><td colspan=2 bgcolor=$colorswitch>$ratinghostname</td><td colspan=3 bgcolor=$colorswitch>$rating</td><td bgcolor=$colorswitch>$formatted_date  </font></b></td><td bgcolor=$colorswitch><center><b><a class=\"rn_csrf\" href=\"".$admin_file.".php?op=LinksDelVote&lid=$lid&rid=$ratingdbid\">X</a></b></center></td></tr><br />";
			$x++;
			if ($colorswitch=="$bgcolor1") $colorswitch="$bgcolor2";
				else $colorswitch="$bgcolor1";
		}
...
function LinksListBrokenLinks() {
	...
	if ($totalbrokenlinks==0) {
		...
		while($row = $db->sql_fetchrow($result)) {
			...
			echo "</td>"
				."<td bgcolor=\"$colorswitch\"><center><a class=\"rn_csrf\" href=\"".$admin_file.".php?op=LinksEditBrokenLinks&amp;lid=$lid\">X</a></center>"
				."<td bgcolor=\"$colorswitch\"><center><a class=\"rn_csrf\" href=\"".$admin_file.".php?op=LinksIgnoreBrokenLinks&amp;lid=$lid\">X</a></center>"
				."</td>"
				."<td bgcolor=\"$colorswitch\"><center><a class=\"rn_csrf\" href=\"".$admin_file.".php?op=LinksDelBrokenLinks&amp;lid=$lid\">X</a></center>"
				."</td>"
				."</tr>";
...
function LinksListModRequests() {
	...
	while($row = $db->sql_fetchrow($result)) {
		...
		echo "<td align=\"right\"><font class=\"tiny\">( <a class=\"rn_csrf\" href=\"".$admin_file.".php?op=LinksChangeModRequests&amp;requestid=$requestid\">" . _ACCEPT . "</a> / <a class=\"rn_csrf\" href=\"".$admin_file.".php?op=LinksChangeIgnoreRequests&amp;requestid=$requestid\">" . _IGNORE . "</a> )</font></td></tr></table>";
	}
...
function LinksDelCat($cid, $sid, $sub, $ok=0) {
	...
	if($ok==1) {
		...
		echo '<b>' . _EZTHEREIS . ' '.$nblink.' ' . _LINK . ' ' . _EZATTACHEDTOCAT . '</b><br />';
		echo '<b>' . _DELEZLINKCATWARNING . '</b><br /><br />';
	}
	echo '[ <a class="rn_csrf" href="'.$admin_file.'.php?op=LinksDelCat&amp;cid='.$cid.'&amp;sid='.$sid.'&amp;sub='.$sub.'&amp;ok=1">' . _YES . '</a> | <a href="'.$admin_file.'.php?op=Links">' . _NO . '</a> ]</font></center><br /><br />';
	CloseTable();
	include_once('footer.php');
}
...
function LinksAddEditorial($linkid, $editorialtitle, $editorialtext) {
	...
	echo "$linkid, $aid, $editorialtitle, $editorialtext</font></center>";
	CloseTable();
	include_once("footer.php");
}
...
function LinksModEditorial($linkid, $editorialtitle, $editorialtext) {
	...
	echo "<br /><center>"
		...
		."[ <a href=\"".$admin_file.".php?op=Links\">" . _WEBLINKSADMIN . "</a> ]<br /><br /></font></center>";
	CloseTable();
	include_once("footer.php");
}
...
function LinksValidate($cid, $sid, $ttitle) {
	...
	echo "<tr><td bgcolor=\"$bgcolor2\" align=\"center\" width=\"100\"><b>" . _STATUS . "</b></td><td bgcolor=\"$bgcolor2\"><b>" . _LINKTITLE . "</b></td><td bgcolor=\"$bgcolor2\" align=\"center\" width=\"150\"><b>" . _FUNCTIONS . "</b></td></tr>";
	while($row = $db->sql_fetchrow($result)) {
		$lid = intval($row['lid']);
		$title = stripslashes($row['title']);
		...
		if (!@fsockopen($vurl['host'], 80, $errno, $errstr, 15)) {
			echo "<tr><td align=\"center\"><b>&nbsp;&nbsp;" . _FAILED . "&nbsp;&nbsp;</b></td>"
				...
				."<td align=\"center\"><font class=\"content\">&nbsp;&nbsp;[ <a href=\"".$admin_file.".php?op=LinksModLink&amp;lid=$lid\">" . _EDIT . "</a> | <a class=\"rn_csrf\" href=\"".$admin_file.".php?op=LinksDelLink&amp;lid=$lid\">" . _DELETE . "</a> ]&nbsp;&nbsp;</font>"
				."</td></tr>";
		} else {
...
function LinksAddLink($new, $lid, $title, $url, $cat, $description, $name, $email, $submitter) {
	...
	if ($numrows>0) {
		...
		echo "[ <a href=\"".$admin_file.".php?op=Links\">" . _WEBLINKSADMIN . "</a> ]</font></center><br /><br />";
		CloseTable();
		if ($new==1) {
			$db->sql_query("delete from " . $prefix . "_links_newlink where lid='$lid'");
			if ($email) {
