...
function index() {
	...
	while ($row = $db->sql_fetchrow($result)) {
		...
		echo '<td valign="top" width="45%"><font class="option"><strong><big>&middot;</big></strong> <a href="modules.php?name=Web_Links&amp;l_op=viewlink&amp;cid='.$cid.'"><b>'.$title.'</b></a></font>';
	...
	echo '<br /><br /><center>'._THEREARE.' <b>'.$numrows.'</b> '._LINKS.' '._AND.' <b>'.$catnum.'</b> '._CATEGORIES.' '._INDB.'</center>'
		. '</div>';
	CloseTable();
...
function AddLink() {
	...
	if ($db->sql_numrows($result) < 1) {
		...
		if (is_user($user) || $links_anonaddlinklock == 1) {  /* 06-24-01 Bug fix : changed $links_anonaddlinklock != 1 to $links_anonaddlinklock == 1 */
			echo '<p><b>'._INSTRUCTIONS.':</b></p><ul>'
				.'<li>'._SUBMITONCE.'</li>'
				.'<li>'._POSTPENDING.'</li>'
				.'<li>'._USERANDIP.'</li>'
				.'</ul><form method="post" action="modules.php?name='.$module_name.'&amp;l_op=Add">'
				.'<b>'._PAGETITLE.':</b> <input type="text" name="title" size="50" maxlength="100" /><br />'
				.'<b>'._PAGEURL.':</b> <input type="text" name="url" size="50" maxlength="100" value="http://" /><br />'
			...
			echo '</select><br /><br />'
				.'<b>'._LDESCRIPTION.'</b><br /><textarea name="description" cols="60" rows="5"></textarea><br /><br />'
				.'<b>'._YOURNAME.':</b> <input type="text" name="auth_name" size="30" maxlength="60" /><br />'
				.'<b>'._YOUREMAIL.':</b> <input type="text" name="email" size="30" maxlength="60" /><br /><br />';
			/*****[BEGIN]******************************************
...
function NewLinksDate($selectdate) {
	...
	while ($row2 = $db->sql_fetchrow($result2)) {
		...
		echo '<b>'._ADDEDON.':</b> '.$datetime.' <b>'._HITS.':</b> '.$hits;
...
function MostPopular($ratenum, $ratetype) {
	...
	if ($mostpoplinkspercentrigger == 1) {
		echo '<font class="option"><b>'._MOSTPOPULAR.' '.$toplinkspercent.'% ('._OFALL.' '.$totalmostpoplinks.' '._LINKS.')</b></font>';
	} else {
		echo '<font class="option"><b>'._MOSTPOPULAR.' '.htmlentities($mostpoplinks).'</b></font>';
	}
...
function viewlink($cid, $min, $orderby, $show) {
	...
	while($row2 = $db->sql_fetchrow($result2)) {
		...
		echo '<td valign="top" width="45%"><font class="option"><strong><big>&middot;</big></strong> <a href="modules.php?name=Web_Links&amp;l_op=viewlink&amp;cid='.$cid2.'"><b>'.$title2.'</b></a></font>';
	...
	echo '<center>'._SORTLINKSBY.': '
		...
		.'<br /><b>'._SITESSORTED.': '.$orderbyTrans.'</b></center><br /><br />';
	if(!is_numeric($min)){
		$min=0;
	}
...
function search($query, $min, $orderby, $show) {
	...
	if ($query != '') {
		if ($nrows>0) {
			...
			echo '<br /><table width="100%" bgcolor="'.$bgcolor2.'"><tr><td><span class="option"><b>'._LINKS.'</b></span></td></tr></table>';
			$orderbyTrans = convertorderbytrans($orderby);
			...
			while($row = $db->sql_fetchrow($result)) {
				...
				if ($parentid3>0) $title3 = getparent($parentid3,$title3);
				echo '<b>'._CATEGORY.':</b> '.$title3.'<br /><br />';
				$x++;
			}
			$orderby = convertorderbyout($orderby);
		} else {
...
function viewlinkeditorial($lid) {
	...
	echo '<br />';
	OpenTable();
	echo '<div class="content" align="center"><font class="option"><b>'._LINKPROFILE.': '.stripslashes(check_html($displaytitle,'nohtml')).'</b></font><br />';
	linkinfomenu($lid);
	if ($recordexist != 0) {
		while($row = $db->sql_fetchrow($result)) {
			$adminid = $row['adminid'];
			...
			echo '<br /><br />';
			OpenTable2();
			echo '<font class="option"><b>'.$editorialtitle.'</b></font><br /><br />'
...
function viewlinkcomments($lid) {
	...
	$displaytitle = $transfertitle;
	OpenTable();
	echo '<div class="content" align="center"><font class="option"><b>'._LINKPROFILE.': '.stripslashes(check_html($displaytitle,'nohtml')).'</b></font><br /><br />';
	linkinfomenu($lid);
	...
	while($row = $db->sql_fetchrow($result)) {
		...
		echo '<tr><td bgcolor="'.$bgcolor2.'" align="left">'
			.'<b> '._USER.': </b><a href="'.$nukeurl.'/modules.php?name=Your_Account&amp;op=userinfo&amp;username='.$ratinguser.'">'.$ratinguser.'</a>'
			.'</td>'
...
function viewlinkdetails($lid) {
	...
	echo '<div class="content" align="center"><font class="option"><b>'._LINKPROFILE.': '.stripslashes(check_html($displaytitle,'nohtml')).'</b></font><br /><br />';
	linkinfomenu($lid);
	echo '<br /><br /><b>'._LINKRATINGDET.'</b><br />'
...
function modifylinkrequest($lid) {
	...
	if ($blocknow != 1) {
		...
		echo '<center><font class="option"><b>'._REQUESTLINKMOD.'</b></font><br /><br />';
		...
		echo '<b>'._LINKID.':</b> '.$lid.'</center><br /><br />'
			.'<form action="modules.php?name='.$module_name.'" method="post">'
			.'<b>'._LINKTITLE.':</b><br /><input type="text" name="title" value="'.$title.'" size="50" maxlength="100" /><br /><br />'
			.'<b>'._URL.':</b><br /><input type="text" name="url" value="'.$url.'" size="50" maxlength="100" /><br /><br />'
			.'<b>'._DESCRIPTION.':</b> <br /><textarea name="description" cols="60" rows="10">'.$description.'</textarea><br /><br />';
		echo '<input type="hidden" name="lid" value="'.$lid.'" />'
...
function ratelink($lid, $user) {
	...
	echo '<div class="content"><b>'.htmlentities(ereg_replace('_', ' ', stripslashes($ttitle))).'</b>'
		.'<ul>'
		.'<li>'._RATENOTE1.'</li>'
		.'<li>'._RATENOTE2.'</li>'
		.'<li>'._RATENOTE3.'</li>'
