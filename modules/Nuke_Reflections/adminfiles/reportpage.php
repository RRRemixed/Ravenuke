...
echo "<br><br><center><strong><u>Admin Reported Files List</u></strong><br><br>";


if ($ignore != "") {
$db->sql_query("update " . $user_prefix . "_reflections_files set approved='1', totalreports='0' where picid='$ignore'");
$db->sql_query("delete from " . $user_prefix . "_reflections_reports where picid='$ignore'");
echo "<hr><center><strong>Reports have been deleted on that file.</strong></center><hr>";
}

if ($somethinggood != "") {
	echo "<hr><center><strong>File was Deleted</strong></center><hr>";

}

if ($moreon != "") {
echo "<table border=\"1\" width=\"100%\"><tr><td>";
echo "<strong>More Information About Reported file #$moreon<br><bR>";
$sql = "SELECT * FROM " . $prefix . "_reflections_files WHERE picid='$moreon'";
...
echo "Uploaded by $fileupnick with the IP Address of $fileip<br>";
if ($approved != "1") {
	echo "This file is NOT Approved<br><br>";
} else {
	echo "This file is Approved<br><bR>";
}


$sql1 = "SELECT * FROM " . $prefix . "_reflections_reports WHERE picid='$filepicid'";
$result1 = mysql_query($sql1) or die ('SQL Select Failed!!');
$num1 = mysql_numrows($result1);
$a = 0;
while ($a < $num1) {
    $reportid = mysql_result($result1, $a, "reportid");

    $galid = mysql_result($result1, $a, "galid");
        $reportby = mysql_result($result1, $a, "reportby");
            $reportbyip = mysql_result($result1, $a, "reportbyip");
                $date = mysql_result($result1, $a, "date");
                    $time = mysql_result($result1, $a, "time");
                        $rawtime = mysql_result($result1, $a, "rawtime");
                            $comment = mysql_result($result1, $a, "comment");
if ($comment == "") {
	$comment = "No Comment was Left";
}

echo "</center>";
echo "Report #$a was reported by $reportby with ip of $reportbyip<br>
Comment on report :: $comment<br>
---------------------------------------------------------------<br>";
...
echo "<tr>";
$tr = "0";
$sql = "SELECT * FROM " . $prefix . "_reflections_files WHERE totalreports!='0' ORDER BY `rawtime` DESC";
$result = mysql_query($sql) or die ('SQL Select Failed!!');
$num = mysql_numrows($result);
$i = 0;
while ($i < $num) {
...
echo "Thumbnail Exists <img src='modules/$module_name/images/okno.gif'><br>";
 ...
 echo "<strong>Total Reports :: $filetotalreports</strong><bR>";

echo "<a href=\"modules.php?name=$module_name&adminarea=reportpage&moreon=$filepicid\">[See More on Reports]</a><br>";
echo "<br>";
echo "<a href=\"modules.php?name=$module_name&adminarea=reportpage&ignore=$filepicid\">[Ignore]</a>";
echo " || ";
echo "<a href=\"http://devil-modz.us/modules.php?name=$module_name&adminarea=admindeletefile&deletenow=yes&fileid=$filepicid&goback=report\">[Delete]</a>";
