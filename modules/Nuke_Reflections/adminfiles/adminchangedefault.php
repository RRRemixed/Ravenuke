...
$checkfilename = $row['filename'];
$checkinfolder = $row['infolder'];
if ($checkgalid1 == $usergalid) {

if ($usergalthumb != "") {
$checkold = "modules/$module_name/files/maingallery/$usergalfolder/$usergalthumb";
if (file_exists($checkold)) {
		if (!unlink($checkold)) {
    echo "<br><b><center>There was an error Deleting the old thumbnail at $checkold. Must be a permission error!!</b></center><br>";
			}
	}
}
...
if ($checkfilename != "") {
$checknewfile = "modules/$module_name/files/maingallery/$checkinfolder/fullsize/$checkfilename";
$uploadFile = "modules/$module_name/files/maingallery/$checkinfolder/$checkfilename";
if (file_exists($checknewfile)) {
copy($checknewfile, $uploadFile);
...
wdcreatetgalhumb($origionalpic, $thumbnailpic, $imgSize[0], $imgSize[1]);
    ...
    echo "<br><b><center>There was an error. The file you selected does not exist on the server!</b></center><br>";
$ajerror = "1";
}
}
...
$db->sql_query("update " . $user_prefix . "_reflections_gallery set thumb='$checkfilename' where galid='$usergalid'");

    echo "<br><b><center>Default Gallery Image has been updated. Thanks</b></center><br><hr>";
$usergalthumb = $checkfilename;



} else {
    echo "<br><b><center>There was an error. Look above for information!!</b></center><br><hr>";
...
echo "<a href='modules.php?name=$module_name&adminarea=adminchangedefault&set=firstpic&galid=$usergalid'><u>[Click Here]</u></a> To set as first pick from gallery that is approved?";
echo "<hr>OR<hr>";
echo "Click an image below to set it as a default<br>
<i>Notice only approved files will show</i><br>";
