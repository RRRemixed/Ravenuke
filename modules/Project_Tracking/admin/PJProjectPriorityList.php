...
$pagetitle = ': ' . _PJ_TITLE . ': ' . _PJ_PROJECTS . ': ' . _PJ_PRIORITYLIST;

include('header.php');

pjadmin_menu(_PJ_PROJECTS . ': ' . _PJ_PRIORITYLIST);
echo '<br />'."\n";

$priorityresult = $db->sql_query('SELECT * FROM ' . $prefix . '_nsnpj_projects_priorities WHERE priority_weight > 0 ORDER BY priority_weight');
$priority_total = $db->sql_numrows($priorityresult);

OpenTable();
echo '<table width="100%" border="1" cellspacing="0" cellpadding="2">'."\n";
echo '<tr><td colspan="3" width="100%" bgcolor="' . $bgcolor2 . '" nowrap="nowrap"><b>' . _PJ_PRIORITYOPTIONS . '</b></td></tr>'."\n";

$pjimage = pjimage('options.png', $module_name);
echo '<tr><td><img src="' . $pjimage. '" alt="" title="" /></td><td colspan="2" width="100%" nowrap="nowrap"><a href="' . $admin_file . '.php?op=PJProjectPriorityAdd">' . _PJ_PRIORITYADD . '</a></td></tr>'."\n";

$pjimage = pjimage('stats.png', $module_name);
echo '<tr><td><img src="' . $pjimage. '" alt="" title="" /></td><td colspan="2" width="100%" nowrap="nowrap">' . _PJ_TOTALPROJECTPRIORITIES . ': <b>' . $priority_total . '</b></td></tr>'."\n";
echo '</table>'."\n";
echo '<br />'."\n";
echo '<table width="100%" border="1" cellspacing="0" cellpadding="2">'."\n";
echo '<tr><td colspan="2" bgcolor="' . $bgcolor2 . '" width="100%"><b>' . _PJ_PRIORITIES . '</b></td>'."\n";
...
if($priority_total != 0) {
	while($priority_row = $db->sql_fetchrow($priorityresult)) {
		...
		echo '</td>'."\n";
		echo '<td align="center" nowrap="nowrap">[ <a href="' . $admin_file . '.php?op=PJProjectPriorityEdit&amp;priority_id=' . $priority_row['priority_id'] . '">' . _PJ_EDIT . '</a>';
		echo ' | <a href="' . $admin_file . '.php?op=PJProjectPriorityRemove&amp;priority_id=' . $priority_row['priority_id'] . '">' . _PJ_DELETE . '</a> ]</td></tr>'."\n";
	}

	echo '<tr><td align="center" colspan="4"><a class="rn_csrf" href="' . $admin_file . '.php?op=PJProjectPriorityFix">' . _PJ_FIXWEIGHT . '</a></td></tr>'."\n";
} else {
	echo '<tr><td width="100%" colspan="4" align="center">' . _PJ_NOPROJECTPRIORITY . '</td></tr>'."\n";
}
