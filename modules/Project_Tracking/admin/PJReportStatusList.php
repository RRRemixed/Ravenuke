...
$pagetitle = ': ' . _PJ_TITLE . ': ' . _PJ_REPORTS . ': ' . _PJ_STATUSLIST;
...
echo '<tr><td><img src="' . $pjimage . '" alt="" title="" /></td><td colspan="2" width="100%" nowrap="nowrap"><a href="' . $admin_file . '.php?op=PJReportStatusAdd">' . _PJ_STATUSADD . '</a></td></tr>'."\n";

$pjimage = pjimage('stats.png', $module_name);
echo '<tr><td><img src="' . $pjimage . '" alt="" title="" /></td><td colspan="2" width="100%" nowrap="nowrap">' . _PJ_TOTALSTATUSES . ': <b>' . $status_total . '</b></td></tr>'."\n";
echo '</table>'."\n";
echo '<br />'."\n";
echo '<table width="100%" border="1" cellspacing="0" cellpadding="2">'."\n";
echo '<tr><td colspan="2" bgcolor="' . $bgcolor2 . '" width="100%"><b>' . _PJ_STATUSLIST . '</b></td>';
...
if($status_total != 0) {
	while($status_row = $db->sql_fetchrow($statusresult)) {
		...
		if($pid1 AND $pid1 > 0) {
			echo '<a class="rn_csrf" href="' . $admin_file . '.php?op=PJReportStatusOrder&amp;weight=' . $status_row['status_weight'] . '&amp;pid=' . $status_row['status_id'] . '&amp;weightrep=' . $weight1 . '&amp;pidrep=' . $pid1 . '"><img src="modules/' . $module_name . '/images/weight_up.png" border="0" hspace="3" alt="' . _PJ_UP . '" title="' . _PJ_UP . '" /></a>';
		} else {
			echo '<img src="modules/' . $module_name . '/images/weight_up_no.png" border="0" hspace="3" alt="" title="" />';
		}

		if($pid2) {
			echo '<a class="rn_csrf" href="' . $admin_file . '.php?op=PJReportStatusOrder&amp;weight=' . $status_row['status_weight'] . '&amp;pid=' . $status_row['status_id'] . '&amp;weightrep=' . $weight3 . '&amp;pidrep=' . $pid2 . '><img src="modules/' . $module_name . '/images/weight_dn.png" border="0" hspace="3" alt="' . _PJDOWN . '" title="' . _PJ_DOWN . '" /></a>';
		} else {
			echo '<img src="modules/' . $module_name . '/images/weight_dn_no.png" border="0" hspace="3" alt="" title="" />';
		}

		echo '</td>'."\n";
		echo '<td align="center" nowrap="nowrap">[ <a href="' . $admin_file . '.php?op=PJReportStatusEdit&amp;status_id=' . $status_row['status_id'] . '">' . _PJ_EDIT . '</a>';
		echo ' | <a href="' . $admin_file . '.php?op=PJReportStatusRemove&amp;status_id=' . $status_row['status_id'] . '">' . _PJ_DELETE . '</a> ]</td></tr>'."\n";
	}

	echo '<tr><td align="center" colspan="4"><a class="rn_csrf" href="' . $admin_file . '.php?op=PJReportStatusFix">' . _PJ_FIXWEIGHT . '</a></td></tr>'."\n";
} else {
