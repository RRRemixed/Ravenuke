...
$pagetitle = ": "._PJ_TITLE.": "._PJ_REQUESTS.": "._PJ_IMPORTASTASK;
include("header.php");
...
while(list($s_project_id, $s_project_name) = $db->sql_fetchrow($projectlist)){
    if($s_project_id == $request['project_id']){ $sel = "selected='selected'"; } else { $sel = ""; }
...
echo "<td><textarea name='task_description' cols='60' rows='10'>".$request['request_description']."</textarea></td></tr>\n";
...
for($i = 1; $i <= 12; $i++){
    if($i == date("m")){ $sel = "selected='selected'"; } else { $sel = ""; }
    echo "<option value='$i' $sel>$i</option>\n";
}
echo "</select><select name='task_start_day'><option value='00'>--</option>\n";
for($i = 1; $i <= 31; $i++){
    if($i == date("d")){ $sel = "selected='selected'"; } else { $sel = ""; }
    echo "<option value='$i' $sel>$i</option>\n";
}
