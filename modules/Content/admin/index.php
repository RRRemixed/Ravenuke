...
global $db, $prefix, $ver, $admin_file, $module_name, $locale, $datetime;
	...
	echo '	<tr>'.PHP_EOL;
	echo '		<td>'.$bullet.' <b>'._CP_DATELASTADD.'</b>:</td><td>'.$datetime.'</td>'.PHP_EOL;
	echo '	</tr>'.PHP_EOL;
	echo '</table>'.PHP_EOL;
	CloseTable();
	include('footer.php');
...
global $db, $prefix, $admin_file, $bgcolor4, $ver, $module_name;
	...
	while($row = $db->sql_fetchrow($result)) {
		...
		echo '		<td width="75%" valign="top"><b>'.$title.'</b><br />'.substr($description, 0, 255).'...</td>'.PHP_EOL;
...
global $admin_file, $module_name, $ver;
	...
	echo '<b>'._CP_UPLOADNEWIMG.'</b>:<br />'.PHP_EOL;
...
global $db,$prefix, $admin_file, $bgcolor4, $module_name, $ver;
    ...
    if ($numrows>0) {
		...
		while (list($pid, $cid, $title, $uname) = $db->sql_fetchrow($result)) {
			...
			$ctitle = $urow['title'];
			echo '<tr style="background-color: '.$bgcolor4.';"><td><big>&middot;</big> <a href="'.$admin_file.'.php?op=CPAddNewPage&amp;pid='.$pid.'">'.$title.'</a></td><td align="center">'.$uname.'</td>'.PHP_EOL;
...
function CPAddNewPage($pid) {
    ...
    $date = $row['date'];
    $clanguage = intval($row['clanguage']);
    $uname = $row['uname'];
	...
	if ($multilingual == 1) {
		...
		//echo '<b>'._CP_LANGUAGE.': '.ucfirst($clanguage).'</b><br /><br />'.PHP_EOL;
...
function CPDelete($pid, $ok=0) {
    ...
    if ($ok==1) {
		...
		$title = $row['title'];
        OpenTable();
		echo '<div style="text-align: center;"><b>'._CP_DELCONTENT.': '.$title.'</b><br /><br />'
...
<em>MERCHANTABILITY</em> or <em>FITNESS FOR A PARTICULAR PURPOSE</em>. See the <a href="http://www.gnu.org/licenses/gpl.html" target="new"><b>GNU General Public License</b></a> for more details.</p>
...
<p>* <b>Francisco Burzi</b>: Creator of the original module &amp; former PHP-Nuke developer (<a href="http://www.phpnuke.org">www.phpnuke.org</a>)<br />
* <b>Chatserv</b>: Creator of the Chatserv Content Hack for PHP-Nuke 7.5 (<a href="http://www.nukeresources.com">www.nukeresources.com</a>)<br />
* <b>Silvestre</b>: Designer of old icons (<a href="http://www.silvestre.com.ar">www.silvestre.com.ar</a>)<br />
* <b>Trubador</b>: Bugfixing (<a href="http://yrf.org.uk">http://yrf.org.uk</a>)<br />
* <b>THoTH</b>: Bugfixing and brainstorming (<a href="http://www.book-of-thoth.com">http://www.book-of-thoth.com</a>)<br />
* <b>Vecchio Joe</b>: Heavy testing, bugfixing, brainstorming and italian translation (<a href="http://vecchiojoe.it">http://vecchiojoe.it</a>)<br />
* <b>GANYANCI</b>: Heavy testing, bugfixing and turkish translation (<a href="http://alberbalci.com">http://alperbalci.com</a>)<br />
* <b>Function Design &amp; Development Studio</b>: Creators of the new icon theme (<a href="http://www.wefunction.com">http://www.wefunction.com</a>)<br />
* <b>RavenNuke Team</b>: For including the module as part of your distro and for your feedback and bug reporting/fixing (<a href="http://www.ravenphpscripts.com">www.ravenphpscripts.com</a>)<br />
* <b>All Nukers around the world</b>: you are who made this possible</p>
<p style="text-align: center;"><b>'._CP_GOBACK.'</b></p>';
CloseTable();
include('footer.php');
}
...
global $db, $prefix, $module_name, $ThemeSel, $ver;
	...
	echo '<br /><br />'.PHP_EOL;
	echo '<center>[ <a href="javascript:window.close();"><b>'._CP_CLOSEWINDOW.'</b></a> ]</center>'.PHP_EOL;
	CloseTable();
	echo '</body>'.PHP_EOL;
	echo '</html>'.PHP_EOL;
}
...
switch ($op) {
	...
	include('header.php');
	OpenTable();
	echo '<center><b>'._CP_ERROR.'</b><br /><br />You do not have administration permissions for module "'.$module_name.'"!</center>';
	CloseTable();
	include('footer.php');
}
?>