...
function featured_content($cid) {
	...
	if($numrows>0) {
		...
		echo '<center><b>'._CP_CPFEATCONTENT.'</b></center><br />'.PHP_EOL;
		...
		echo '	<a href="modules.php?name='.$module_name.'&amp;pa=showpage&amp;pid='.$fpid.'"><b>'._CP_CPREADMORE.'</b></a> ('.$fhits.' '._CP_READS.')</div>'.PHP_EOL;
...
function print_page($pid) {
	...
	if($myactive==0 && !is_admin($admin)) {
		include('header.php');
		OpenTable();
		echo '<center>'._CP_NOCONTENTID.'</center>'.PHP_EOL;
		CloseTable();
		include('footer.php');
	} else {
...
function PrintPDF($pid) {
	...
	$mypage_header = stripslashes($mypage['page_header']);
	$mytext = stripslashes($mypage['text']);
	//$mytext = ereg_replace("<!--pagebreak-->", "<br /><br />", $mytext);
	$mytext = str_replace('[--pagebreak--]', '<br /><br />', $mytext);
	$mypage_footer = stripslashes($mypage['page_footer']);
	$mysignature = stripslashes($mypage['signature']);
	$mydate = $mypage['date'];
	...
	$html = '<b>'.$mytitle.'</b><br />'.$mysubtitle.'<br /><br />';
	If (!empty($mypage_header)) {
	...
	$html.= '<b>'._CP_COMESFROM.'</b><br />';
	$html.= $sitename.':<br />';
	$html.= '<a href="'.$nukeurl.'">'.$nukeurl.'</a><br /><br />';
	$html.= '<b>'._CP_THEURL.'</b><br />';
...
function add_page() {
	...
	if (isset($_POST['uname'])) {$uname = real_escape_content(check_html($_POST['uname'], 'nohtml'));} else {$uname='';}
		...
		if($error===1) {
			echo '<div style="text-align: center;"><b>ERROR</b>: '._CP_CPADDERROR.'<br /><br />'.PHP_EOL;
			...
			cpheader();
			OpenTable();
			echo '<br /><div style="text-align: center;">'._CP_TTHANKS.' <b>'.$uname.'</b><br />'.PHP_EOL;
		...
		cpheader();
		OpenTable();
		echo '<div style="text-align: center;"><b>'._CP_ADDNEWPAGE.'</b></div><br />'.PHP_EOL;
		echo '<form action="modules.php?name='.$module_name.'&amp;pa=preview_page" method="post">'.PHP_EOL;
		if (is_user($user)) {
			echo '<b>'._CP_USERNAME.':</b>'.PHP_EOL;
			cookiedecode($user);
		...
		echo '</select><br /><br />';
		echo '<b>'._CP_HEADERTEXT.':</b><br />'.PHP_EOL;
		...
		echo '<br /><br />'.PHP_EOL;
		echo '<b>'._CP_PAGETEXT.':</b><br />'.PHP_EOL;
		...
		echo '<br /><br />'.PHP_EOL;
		echo '<b>'._CP_FOOTERTEXT.':</b><br />'.PHP_EOL;
		if (function_exists('wysiwyg_textarea')) {
		...
		echo '<br /><br />'.PHP_EOL;
		echo '<b>'._CP_SIGNATURE.':</b><br />'.PHP_EOL;
		if (function_exists('wysiwyg_textarea')) {
		...
		if ($multilingual == 1) {
			echo '<b>'._CP_LANGUAGE.': </b><br />'.PHP_EOL;
...
function preview_page() {
	...
	if($error===1) {
		...
		echo $html;
		echo '<b>'._CP_CPISFINE.'</b><br /><br />'.PHP_EOL;
...
function BrowseTag($tag, $order, $ofsbgn, $ofsppg) {
	...
	echo '	<tr>'.PHP_EOL;
	echo '		<td width="80%" align="right" colspan="2"><b>'._CP_SORTBY.'</b>'.PHP_EOL;
	...
	$result = $db->sql_query('SELECT * FROM '.$prefix.'_pages WHERE (tags like \'%'.$tag.'%\') ORDER BY '.$orderby.' WHERE active=1 LIMIT '.$ofsbgn.','.$ofsppg.'');
	while($row = $db->sql_fetchrow($result)) {
		$pid = intval($row['pid']);
		...
		if (is_admin($admin)) {
			...
			echo '<td width="80%"><b><big>&middot;</big></b> '.$the_lang.'&nbsp;'.PHP_EOL;
			...
			echo '<tr style="background-color: '.$bgcolor4.';">'.PHP_EOL;
			echo '<td width="85%"><big><b>&middot;</b></big> '.$the_lang.'&nbsp;'.PHP_EOL;
			echo '<a href="modules.php?name='.$module_name.'&amp;pa=showpage&amp;pid='.$pid.'">'.$title.'</a> '.$subtitle.PHP_EOL;
			newcontentgraphic($date);
			popgraphic($counter);
			echo '</td>'.PHP_EOL;
