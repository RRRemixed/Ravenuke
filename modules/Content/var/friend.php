...
global $user, $cookie, $prefix, $db, $user_prefix, $module_name;
	...
	cpheader();
	OpenTable();
	echo '<center><span class="content"><b>'._CP_FRIEND.'</b></span></center><br /><br />'.PHP_EOL;
	echo _CP_YOUSENDSTORY.' <b>'.$title.'</b> '._CP_TOAFRIEND.'<br /><br />'.PHP_EOL;
	...
	echo '<b>'._CP_FYOURNAME.' </b> <input type="text" name="yname" value="'.$yn.'" /><br /><br />'.PHP_EOL;
	echo '<b>'._CP_FYOUREMAIL.' </b> <input type="text" name="ymail" value="'.$ye.'" /><br /><br /><br />'.PHP_EOL;
	echo '<b>'._CP_FFRIENDNAME.' </b> <input type="text" name="fname" /><br /><br />'.PHP_EOL;
	echo '<b>'._CP_FFRIENDEMAIL.' </b> <input type="text" name="fmail" /><br /><br />'.PHP_EOL;
...
global $sitename, $slogan, $nukeurl, $prefix, $db, $module_name;
    ...
    if(!empty($fname) && !empty($fmail) && !empty($yname) && !empty($yname)) {
		...
		$message.= '<b>'.$mytitle.'</b><br /><br />'.PHP_EOL;
    	...
    	include('header.php');
   		cpheader();
    	OpenTable();
   		echo '<center>'._CP_FSTORY.' <b>'.$mytitle.'</b>'.PHP_EOL;
		echo ''._CP_HASSENT.' <b>'.$fname.'</b><br /><br />'.PHP_EOL;
