...
global $prefix, $user_prefix, $db, $sitename, $admin, $module_name, $locale, $datetime, $bgcolor4, $nukeurl, $admin_file;
	...
	ereg ('([0-9]{4})-([0-9]{1,2})-([0-9]{1,2}) ([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})', $mydate, $datetime);
	...
	if (($myactive == 0) && (!is_admin($admin))) {
		...
		if($numrows !== 0) {
			...
			echo '<span style="float: left;">'._CP_AUTHOR.': <a href="modules.php?name=Your_Account&amp;op=userinfo&amp;username='.$myuname.'"><b>'.$realname.'</b></a>.</span>'.PHP_EOL;
		} else {
			echo '<span style="float: left;">'._CP_AUTHOR.': <b>'.$myuname.'</b>.</span>'.PHP_EOL;
		}
		...
		if ($page == $pageno) {
			...
			echo '<b>'._CP_CATEGORY.'</b>: '.$category.'<br />'.PHP_EOL;
			if (!empty($mytags)) {
				echo '<b>'._CP_TAGS.'</b>: '.Tag2Link($mytags, $module_name).'<br />'.PHP_EOL;
			} else {
				echo '<b>'._CP_TAGS.'</b>: '._CP_NONE.'<br />'.PHP_EOL;
			}
...
function list_pages() {
	...
	echo '<b>'.$sitename.'</b><br /><br /></center>'.PHP_EOL;
	...
	if ($numrows > 0) {
		echo '<center>'._CP_CONTENTCATEGORIES.'</center><br />'.PHP_EOL;
		...
		while ($row = $db->sql_fetchrow($result)) {
			...
			//if ($numrows3 > 0) {
				...
				echo '			<span style="font-size: 115%;"><b><a href="modules.php?name='.$module_name.'&amp;pa=list_pages_categories&amp;cid='.$cid.'">'.$title.'</a></b> ('.$numrows3.' '._CP_CPARTICLES.')</span><br />'.$description.PHP_EOL;
				echo '		</td>'.PHP_EOL;
				echo '	</tr>'.PHP_EOL;
			//}
		}
	...
	if($numresults > 0) {
		...
		while ($row4 = $db->sql_fetchrow($result4)) {
			...
			if (is_admin($admin)) {
				...
				echo '	<tr style="background-color: '.$bgcolor4.';">'.PHP_EOL;
				echo '		<td width="80%"><big><b>&middot;</b></big> '.$the_lang.'&nbsp;'.PHP_EOL;
				echo '			<a href="modules.php?name='.$module_name.'&amp;pa=showpage&amp;pid='.$pid.'">'.$title.'</a> '.$subtitle.PHP_EOL;
				newcontentgraphic($date);
				popgraphic($counter);
				echo '		</td>'.PHP_EOL;
	...
	if (is_admin($admin)) {
		...
		if($nonactive > 0) {
			echo '<br /><br />'.PHP_EOL;
			echo '<center><b>'._CP_YOURADMINLIST.'</b></center><br /><br />'.PHP_EOL;
...
function list_pages_categories($cid, $ofsbgn, $ofsppg, $order) {
	...
	if (is_admin($admin)) {
		...
		if($numresults > 0) {
			echo '<br /><br /><center><b>'._CP_YOURADMINLIST.'</b></center>'.PHP_EOL;
			echo '<br /><br />'.PHP_EOL;
			while ($row2 = $db->sql_fetchrow($result2)) {
				...
				echo '<b><big>&middot;</big></b> '.$the_lang.'&nbsp;'.PHP_EOL;
				echo '<a href="modules.php?name='.$module_name.'&amp;pa=showpage&amp;pid='.$pid.'">'.$title.'</a> '.$subtitle.' [&nbsp;'.PHP_EOL;
				echo '<a href="'.$admin_file.'.php?op=CPEdit&amp;pid='.$pid.'">'._CP_EDIT.'</a> |&nbsp;'.PHP_EOL;
				echo '<a class="rn_csrf" href="'.$admin_file.'.php?op=CPChangeStatus&amp;pid='.$pid.'&amp;active=0">'._CP_ACTIVATE.'</a> |&nbsp;'.PHP_EOL;
				echo '<a class="rn_csrf" href="'.$admin_file.'.php?op=CPDelete&amp;pid='.$pid.'">'._CP_DELETE.'</a> ]<br />'.PHP_EOL;
