		...
		if ($db->sql_numrows($result) > 0) {
			...
			while ($row = $db->sql_fetchrow($result)) {
				...
				echo '<tr><td>' . $row['id'] . '</td>' .
					...
					'<td><center>' . "<a href=\"$viewAction{$row['id']}\">$viewImg</a>&nbsp;" .
						"<a href=\"$editAction{$row['id']}\">$editImg</a>$rsvp</center></td>" .
					'<td><center><input type="checkbox" name="select[' . $row['id'] . ']" value="1" /></center></td>' .
					'</tr>';
			}
			echo '</table><br />';
			echo $selectLinks;
			...
			echo '<center>' . ($this->approveMode ? _NO_PENDING_EVENTS : _NO_EVENTS) . '</center>';
		}
