...
<br />
END_PURGE_FORM;
		echo '<center><a href="' . GCAL_MOD_LINK . '">' . _ADMIN_GOTO_MODULE . '</a></center><br />';

		CloseTable();
		OpenTable();

		if (array_key_exists($nolink, $titles)) {
			echo '<center><div class="title">' . $titles[$nolink] . '</div></center><br /><br />';
		}
	}
	...
	function confirmDeleteEvent($id) {
		...
		gcalAdminMenu();
		$id = intval($id);
		echo '<center><div class="title">' . sprintf(_ADMIN_CONFIRM_DEL, $id) . '</div></center><br /><br />';
		...
		echo '<center>[ ' . $yesLink . ' ] &nbsp;&nbsp;&nbsp; [ ' . $noLink . ' ]</center><br /><br />';
	}
	...
	function deleteEvent($id) {
		...
		if ($result) {
			echo '<br /><center>' . _ADMIN_DELETE_OK . $id . '</center>';
		} else {
			echo '<br /><center><b>' . _ADMIN_DELETE_ERR . $id . '</b></center>';
		}
		echo '<br /><br />';
	}
	...
	function deleteSelectedEvents() {
		...
		if ($errMsg != '') {
			echo '<br /><center>' . $errMsg . '</center><br /><br />';
		}
...
END_CAT_FORM;
	...
	function purgeEvents() {
		...
		echo '<center><div class="title">' . _ADMIN_PURGE_RESULTS . '</div></center><br /><br />';
		...
		echo '<center>' . ($success ? _ADMIN_DB_OK : _ADMIN_DB_ERR) . '</center><br /><br />';
	}
...
<fieldset>
	...
	<center><input type="submit" name="ok" value="$lUpdate" /></center>
</fieldset>
</form>
END_FORM;
	}
	...
	function saveConfig() {
		...
		$result = $db->sql_query($sql);
		if (!$result) {
			echo '<center><b>' . _ADMIN_DB_ERR . '</b></center><br /><br />';
		}
	}
	...
	function gcalDisplayRsvpEdit($id) {
		...
		if (count($users) > 0) {
			...
			echo '<center><form action="' . $formAction . '" method="post" onsubmit="return gcalRsvpDelConfirm();">';
...
END_RSVP_DEL_CONFIRM;
			...
			echo '<center>' . _GCAL_EMPTY_RSVP_LIST . '</center>';
		}
		echo '<br /><br />';
	}
	...
	function gcalAbout($config) {
		...
		echo '<center><div class="title">GCalendar ' . $config['version'] . '</div></center><br /><br />';
		echo '<center>' . _GCAL_ADMIN_ABOUT_TEXT . '<br /><br />';
	...
	$config = getConfig();
	echo '<center><div class="title">' . _ADMIN_TITLE . ' ' . $config['version'] . '</div></center><br /><br />';
	...
	switch ($op) {
		...
		case 'gcal_edit_event':
			gcalAdminMenu();
			echo '<center><div class="title">' . _ADMIN_EDIT_EVENT . $id . '</div></center><br /><br />';
		...
		case 'gcal_edit_rsvp':
			gcalAdminMenu();
			$id = intval($_GET['id']);
			echo '<center><div class="title">' . _GCAL_ADMIN_EDIT_RSVP . $id . '</div></center><br /><br />';
		...
		case 'gcal_update_event':
			...
			if (array_key_exists('delete', $_POST)) {
				...
				if ($eventForm->errorCount() > 0) {
					...
					gcalAdminMenu($noLink);
					if (!$success) {
						echo '<center>' . _ADMIN_UPDATE_ERR . $id . '</center><br /><br />';
					}
					$table = new AdminEventTable(($noLink == 'approve') ? 'start_date' : 'id',
												$noLink == 'approve',
												$admin_file . '.php?op=' . $backTo,
		...
		case 'gcal_adding_event':
			...
			if ($eventForm->errorCount() > 0) {
				...
				gcalAdminMenu();
				echo '<center><div class="title">' . _ADMIN_ADD_EVENT . '</div></center><br /><br />';
				$success = $eventForm->save();
				echo '<center>' . ($success ? _ADMIN_ADD_OK : _ADMIN_ADD_ERR) . '</center><br /><br />';
			}
			break;
		...
		case 'gcal_view_event':
			$id = intval($id);
			gcalAdminMenu();
			echo '<center><div class="title">' . _ADMIN_VIEW_EVENT . $id . '</div></center><br /><br />';
			...
			echo '<a href="' . $editLink . '">' . _ADMIN_EDIT_EVENT_LINK . '</a></center><br /><br />';
			echo '<center>:: <a href="javascript:history.back()">' . _GCAL_GO_BACK . '</a> ::</center><br />';
			break;
	}
