...
if (gcalCanSubmit($config['user_submit'], $config['groups_submit'])) {
	...
	if (empty($op)) {
		...
		if ($eventForm->errorCount() > 0) {
			...
			$reqApproval = $config['req_approval'] && !gcalNeedsNoApproval($config['groups_no_approval']);

			if ($success) {
				echo '<center><b>' . _THANKS_SUBMISSION . '</b><br /><br />';
				...
				echo '<center>' . _SUBMIT_ERROR . '</center><br /><br />';
			}
			echo '<center><a href="' . GCAL_MOD_LINK . '">' . $config['title'] . '</a></center><br /><br />';
		}
	}
} else {
	echo '<center>' . _SUBMIT_DISABLED . '</center><br /><br />';
}
