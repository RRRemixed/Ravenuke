...
function sendToFriendForm($year, $month, $day, $event) {
	...
	if ($row = $db->sql_fetchrow($result)) {
		...
		if ($row = $db->sql_fetchrow($result)) {
			...
			gcalFriendHdr();
			echo _YOU_WILL_SEND_EVENT . '<b>' . $title . '</b><br /><br />';
			...
			echo _YOUR_NAME . '&nbsp;<b>' . $name .
				'</b><input type="hidden" name="member" value="' . $name . '" /><br /><br />';
			echo _YOUR_EMAIL . '&nbsp;<b>' . $email .
				'</b><input type="hidden" name="from" value="' . $email . '" /><br /><br />';
			...
			echo '</form>';
			echo '<center>[ <a href="javascript:history.go(-1);">' . _GCAL_GO_BACK . '</a> ]</center>';
			gcalFriendFtr();
		}
	}
}
...
function sendEventToFriend() {
	...
	if (!gcalValidEmail($from) || !gcalValidEmail($to)) 	{
		gcalFriendHdr();
		echo '<center><b>' . _INVALID_EMAIL . '</b><br /><br />' .
			'[ <a href="javascript:history.go(-1);">' . _GCAL_GO_BACK . '</a> ]</center><br /><br />';
		gcalFriendFtr();
		return;
	}
	$sql = 'SELECT title FROM ' . $prefix . GCAL_EVENT_TABLE . ' WHERE id = ' . $event;
...
function eventSent() {
	...
	gcalFriendHdr();
	echo '<center>' . _FRIEND_EVENT_SENT . '<br /><br />';
	echo '[ <a href="' . $link . '">' . _GCAL_GO_BACK . '</a> ]</center><br />';
	gcalFriendFtr();
}
