...
if (($radminsuper==1) OR ($radminuser==1)) {
  $chng_uid = intval($chng_uid); // waraxe - Reflected XSS
	...
	if ($ya_config['usefb'] == '0') $chng_user_fb = '';
	if ($ya_config['usetw'] == '0') $chng_user_tw = '';
	if ($ya_config['useskype'] == '0') $chng_user_skype = '';
	if ($ya_config['usesteam'] == '0') $chng_user_steam = '';
	...
	$chng_user_fb = ya_fixtext($chng_user_fb);
	$chng_user_tw = ya_fixtext($chng_user_tw);
	$chng_user_skype = ya_fixtext($chng_user_skype);
	$chng_user_steam = ya_fixtext($chng_user_steam);
	...
	if ($stop == '') {
		if ($chng_pass != '') {
			$cpass = md5($chng_pass);
			$db->sql_query('UPDATE '.$user_prefix.'_users SET username=\'' . $chng_uname . '\', name=\'' . $chng_name . '\', user_email=\'' . $chng_email . '\', femail=\'' . $chng_femail . '\', user_website=\'' . $chng_url . '\', user_fb=\'' . $chng_user_fb . '\', user_tw=\'' . $chng_user_tw . '\', user_skype=\'' . $chng_user_skype . '\', user_steam=\'' . $chng_user_steam . '\', user_from=\'' . $chng_user_from . '\', user_occ=\'' . $chng_user_occ . '\', user_interests=\'' . $chng_user_interests . '\', user_viewemail=\'' . $chng_user_viewemail . '\', user_avatar=\'' . $chng_avatar . '\', user_sig=\'' . $chng_user_sig . '\', user_sig_bbcode_uid=\'' . $chng_sig_bbcode_uid . '\', bio=\'' . $chng_bio . '\', user_password=\'' . $cpass . '\', newsletter=\'' . $chng_newsletter . '\', user_attachsig=\'' . $user_attachsig . '\', user_timezone=\'' . $user_timezone . '\', user_lang=\'' . $user_lang . '\', user_dateformat=\'' . $user_dateformat . '\' , user_allowhtml=\'' . $user_allowhtml . '\', user_allowbbcode=\'' . $user_allowbbcode . '\', user_allowsmile=\'' . $user_allowsmile . '\', user_allow_viewonline=\'' . $user_allow_viewonline . '\', user_notify=\'' . $user_notify . '\', user_notify_pm=\'' . $user_notify_pm . '\', user_popup_pm=\'' . $user_popup_pm . '\', user_points=\'' . $chng_points . '\' WHERE user_id=\'' . $chng_uid . '\'');
		} else {
