...
if (($radminsuper==1) OR ($radminuser==1)) {
	...
	if ($ya_config['usefb'] == '0') $add_fb = '';
	if ($ya_config['usetw'] == '0') $add_tw = '';
	if ($ya_config['useskype'] == '0') $add_skype = '';
	if ($ya_config['usesteam'] == '0') $add_steam = '';
	...
	if ($stop == '') {
		...
		$sql .= '(user_id, name, username, user_email, femail, user_website, user_regdate, user_fb, user_tw, user_skype, user_steam, user_from, user_occ, user_interests, user_viewemail, user_avatar, user_avatar_type, user_sig, user_sig_bbcode_uid, user_password, newsletter, broadcast, popmeson';
		$sql .= ', points';
		$sql .= ', agreedtos) ';
		$sql .= 'VALUES (\''.$new_uid.'\', \''.$add_name.'\', \''.$add_uname.'\', \''.$add_email.'\', \''.$add_femail.'\', \''.$add_url.'\', \''.$user_regdate.'\', \''.$add_fb.'\', \''.$add_tw.'\', \''.$add_skype.'\', \''.$add_steam.'\', \''.$add_user_from.'\', \''.$add_user_occ.'\', \''.$add_user_interest.'\', \''.$add_user_viewemail.'\', \'gallery/blank.gif\', \'3\', \''.$add_user_sig.'\', \''.$add_sig_bbcode_uid.'\', \''.$add_pass.'\', \''.$add_newsletter.'\', \'1\', \'0\'';
		$sql .= ', \''.$add_points.'\'';
		$sql .= ',\'1\')';
