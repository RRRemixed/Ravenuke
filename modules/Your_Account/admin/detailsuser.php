...
if (($radminsuper==1) OR ($radminuser==1)) {
	...
	if ($db->sql_numrows($db->sql_query($sql)) > 0) {
		...
		echo '<tr><td>' . _NICKNAME . ':</td><td><b>'.$chnginfo['username'].'&nbsp;</b></td></tr>';
		if ($ya_config['userealname'] >= '1') echo '<tr><td>'._UREALNAME.':</td><td><b>'.$chnginfo['name'].'&nbsp;</b></td></tr>';
		echo '<tr><td>' . _EMAIL . ':</td><td><b><a href="mailto:'.$chnginfo['user_email'].'">'.$chnginfo['user_email'].'</a></b></td></tr>';
		if ($ya_config['usefakeemail'] >= '1') echo '<tr><td>' . _FAKEEMAIL . ':</td><td><b>'.$chnginfo['femail'].'&nbsp;</b></td></tr>';
		...
		if ($ya_config['usewebsite'] >= '1') echo '<tr><td>'._WEBSITE.'</td><td><b>'.$userwebsite.'&nbsp;</b></td></tr>';
		...
		while ($sqlvalue = $db->sql_fetchrow($result)) {
			...
			echo '<tr><td>'.$name_exit.'</td><td><b>'.htmlspecialchars($value).'&nbsp;</b></td></tr>';
		}
		if ($ya_config['usefb'] >= '1') echo '<tr><td>'._FB.':</td><td><b>'.$chnginfo['user_fb'].'&nbsp;</b></td></tr>';
		if ($ya_config['usetw'] >= '1') echo '<tr><td>'._TW.':</td><td><b>'.$chnginfo['user_tw'].'&nbsp;</b></td></tr>';
		if ($ya_config['useskype'] >= '1') echo '<tr><td>'._SKYPE.':</td><td><b>'.$chnginfo['user_skype'].'&nbsp;</b></td></tr>';
		if ($ya_config['usesteam'] >= '1') echo '<tr><td>'._STEAM.':</td><td><b>'.$chnginfo['user_steam'].'&nbsp;</b></td></tr>';
		if ($ya_config['uselocation'] >= '1') echo '<tr><td>'._LOCATION.':</td><td><b>'.$chnginfo['user_from'].'&nbsp;</b></td></tr>';
		if ($ya_config['useoccupation'] >= '1') echo '<tr><td>'._OCCUPATION.':</td><td><b>'.$chnginfo['user_occ'].'&nbsp;</b></td></tr>';
		if ($ya_config['useinterests'] >= '1') echo '<tr><td>'._INTERESTS.':</td><td><b>'.$chnginfo['user_interests'].'&nbsp;</b></td></tr>';
		if ($ya_config['usenewsletter'] >= '1') {
			if ($chnginfo['newsletter'] == 1) { $cnl = _YES; } else { $cnl = _NO; }
			echo '<tr><td>'._NEWSLETTER.':</td><td><b>'.$cnl.'</b></td></tr>';
		}
		if ($ya_config['useviewemail'] >= '1') {
			if ($chnginfo['user_viewemail'] ==1) { $cuv = _YES; } else { $cuv = _NO; }
			echo '<tr><td>'._SHOWMAIL.':</td><td><b>'.$cuv.'</b></td></tr>';
		}
		...
		if ($ya_config['useextrainfo'] >= '1') echo '<tr><td>'._EXTRAINFO.'</td><td><b>'.$chnginfo['bio'].'&nbsp;</b></td></tr>';
		if ($ya_config['usepoints'] >= '1' ) echo '<tr><td>'._YA_POINTS.'</td><td><b>'.$chnginfo['points'].'&nbsp;</b></td></tr>';
		echo '<tr><td>'._REGDATE.':</td><td><b>'.$chnginfo['user_regdate'].'&nbsp;</b></td></tr>';
		$chnginfo['lastsitevisit'] = date("d F Y H:i", $chnginfo['lastsitevisit']);
		echo '<tr><td>'._YA_LASTVISIT.'</td><td><b>'.$chnginfo['lastsitevisit'].'</b></td></tr>';
		...
		echo '<tr><td>'._USERSTATUS.'</td><td><b>'.$online.'</b></td></tr>';
		echo '</table></center>';
