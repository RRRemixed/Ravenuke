...
if (!isset($stop)) {
	...
	//	}
	//}
	$server_port = (!ereg('^([0-9]{1,6})', $_SERVER['REMOTE_PORT'])) ? '' : $_SERVER['REMOTE_PORT']; //RN0001003
	...
	if ($ya_config['requireadmin'] == 0 and $ya_config['useactivate'] == 0) {
		...
		$user_fb = (isset($user_fb)) ? check_html($user_fb, 'nohtml') : '';
		$user_tw = (isset($user_tw)) ? check_html($user_tw, 'nohtml') : '';
		$user_skype = (isset($user_skype)) ? check_html($user_skype, 'nohtml') : '';
		$user_steam = (isset($user_steam)) ? check_html($user_steam, 'nohtml') : '';
		...
		$sql = 'INSERT INTO ' . $user_prefix . '_users '
			...
			. 'username, name, user_email, femail, user_website, user_fb, user_tw, user_skype, user_steam, user_from, user_occ, user_interests, newsletter, user_viewemail, '
			...
			. addslashes($user_website) . '\', \'' . $user_fb . '\', \'' . $user_tw . '\', \'' . $user_skype . '\', \'' . $user_steam . '\', \''
		...
		//	$new_uid = $newest_uid+1;
		//}
		$sql = 'INSERT INTO ' . $user_prefix . '_users_temp (user_id, username, name, user_email, user_password, user_regdate, check_num, time, requestor) VALUES (NULL, \'' . addslashes($ya_username) . '\', \'' . addslashes($ya_realname) . '\', \'' . addslashes($ya_user_email) . '\', \'' . $hashed_pass . '\', \'' . $user_regdate . '\', \'' . $check_num . '\', \'' . $time . '\', \'' . $requestor . '\')';
		$result = $db->sql_query($sql);
		$new_uid = $db->sql_nextid();
		if (isset($nfield)) {
			if ((count($nfield) > 0) AND ($result)) {
	...
	if (!$result) {
		...
		if ($ya_config['sendaddmail'] == 1 AND $ya_config['servermail'] == 1) {
			...
			if ($ya_config['usefb'] > 1) $message .= _YFB . ': ' . $user_fb . "\r\n";
			if ($ya_config['usetw'] > 1) $message .= _YTW . ': ' . $user_tw . "\r\n";
			if ($ya_config['useskype'] > 1) $message .= _YSKYPE . ': ' . $user_skype . "\r\n";
			if ($ya_config['usesteam'] > 1) $message .= _YSTEAM . ': ' . $user_steam . "\r\n";
