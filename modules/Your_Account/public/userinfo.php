...
if ($num == 1) {
	if ($usrinfo['user_level'] > 0) {
		 ...
		 */
		if ($ya_config['usefb'] >= '1' AND (is_user($user) OR is_admin($admin))) {
			if ($usrinfo['user_fb'] == '') $usrinfo['user_fb'] = _YA_NA;
			echo '<tr><td width="30%" align="left">' . _FB . '</td><td width="70%" align="left"><strong>' . $usrinfo['user_fb'] . '</strong></td></tr>' ;
		}
		if ($ya_config['usetw'] >= '1' AND (is_user($user) OR is_admin($admin))) {
			if ($usrinfo['user_tw'] == '') $usrinfo['user_tw'] = _YA_NA;
			echo '<tr><td width="30%" align="left">' . _TW . '</td><td width="70%" align="left"><strong>' . $usrinfo['user_tw'] . '</strong></td></tr>' ;
		}
		if ($ya_config['useskype'] >= '1' AND (is_user($user) OR is_admin($admin))) {
			if ($usrinfo['user_skype'] == '') $usrinfo['user_skype'] = _YA_NA;
			echo '<tr><td width="30%" align="left">' . _SKYPE . '</td><td width="70%" align="left"><strong>' . $usrinfo['user_skype'] . '</strong></td></tr>' ;
		}
		if ($ya_config['usesteam'] >= '1' AND (is_user($user) OR is_admin($admin))) {
			if ($usrinfo['user_steam'] == '') $usrinfo['user_steam'] = _YA_NA;
			echo '<tr><td width="30%" align="left">' . _STEAM . '</td><td width="70%" align="left"><strong>' . $usrinfo['user_steam'] . '</strong></td></tr>' ;
		}
