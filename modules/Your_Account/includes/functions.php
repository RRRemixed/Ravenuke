...
function Show_YA_menu() {
	...
	OpenTable();
	if ($stop) {
		echo '<center><span class="title"><b>' . _LOGININCOR . '</b></span></center>';
	} else {
		echo '<center><span class="title"><b>' . _USERREGLOGIN . '</b></span></center>';
	}
	CloseTable();
	echo '<br />';
	OpenTable();
...
function ya_userCheckB($username) {
	...
	// Need to find a way to include extended characters
	//if ((!$username) || ($username=='') || (ereg("[^À-ÿa-zA-Z0-9_-]",$username))) $stop = "<center>"._ERRORINVNICK."</center><br />";
	if ((!$username) || ($username == '') || (ereg('[^a-zA-Z0-9_-]', $username))) $return = 'false';
	...
	if ($ya_config['bad_nick'] > '') {
		...
		$j = count($BadNickList);
		for ($i = 0;$i < $j;++$i) {
			if (eregi($BadNickList[$i], $username)) $return = 'false';
		}
	}
...
function yapagenums($op, $totalselected, $perpage, $max, $find, $what, $match, $listtype) {
	...
	if ($pages > 1) {
		...
		echo '</select><b> ' . _YA_OF . ' ' . $pages . ' ' . _YA_PAGES . '</b> <input type="submit" value="' . _YA_GO . '" /></td></tr>';
		echo '</table></form>';
	}
}
function disabled() {
