...
$aid = substr($aid, 0, 25);
$row = $db->sql_fetchrow($db->sql_query('SELECT title, admins FROM ' . $prefix . '_modules WHERE title=\'Advertising\''));
...
if ($row2['radminsuper'] == 1 || $auth_user == 1) {
	...
	$ad_admin_menu_main = '<div style="text-align: center;" class="title"><b>' . _ADVERTISINGADMIN . '</b><br /><br />[ <a href="'
	...
	$ad_admin_menu = '<div style="text-align: center;" class="title"><b>' . _ADVERTISINGADMIN . '</b><br /><br />[ <a href="'
...
function add_banner() {
	...
	if ($numrows > 0) {
		...
		echo '</select></td></tr>'
			...
			. '</select></td></tr>'
			. '<tr><td>&nbsp;</td><td><i>Select One of the Three Ad Classes, then proceed to fill out the appropriate information about each class.</i></td></tr>';
			}
			else {
		...
		echo '<center><span class="title"><b>' . _ADDNEWBANNER . '</b></span></center><br /><br />'
...
function BannersAdd($name, $cid, $adname, $imptotal, $imageurl, $clickurl, $alttext, $position, $active, $ad_class, $ad_code, $ad_width, $ad_height) {
	...
	$adname = mysql_real_escape_string(check_html($adname, 'nohtml'));
	$alttext = mysql_real_escape_string(check_html($alttext, 'nohtml'));
	$ad_code = mysql_real_escape_string(check_html($ad_code, 'nocheck'));
	$clickurl = mysql_real_escape_string(check_html($clickurl, 'nohtml'));
	$imageurl = mysql_real_escape_string(check_html($imageurl, 'nohtml'));
...
function BannerAddClient($name, $contact, $email, $login, $passwd, $extrainfo) {
	global $prefix, $db, $admin_file;
	$name = mysql_real_escape_string(check_html($name, 'nohtml'));
	$contact = mysql_real_escape_string(check_html($contact, 'nohtml'));
	$email = mysql_real_escape_string(check_html($email, 'nohtml'));
	$login = mysql_real_escape_string(check_html($login, 'nohtml'));
	$passwd = mysql_real_escape_string(check_html($passwd, 'nohtml'));
	$extrainfo = mysql_real_escape_string(check_html($extrainfo, 'nocheck'));
...
function BannerClientDelete($cid, $ok = 0) {
	...
	if ($ok == 1) {
		...
		OpenTable();
		echo '<div style="text-align: center;"><b>' . _DELETECLIENT . ': ' . $name . '</b><br /><br />
...
function ad_positions() {
	...
	echo '<div style="text-align: center;" class="title">' . _ADDNEWPOSITION . '<br /><br />'
		...
		. _POSITIONNAME . ': <input type="text" name="ad_position_name" /> ' . _POSITIONNUMBER . ': <b>'
		. $pos_num . '</b><input type="hidden" name="ad_position_number" value="'
		. $pos_num . '" /><input type="hidden" name="position_new" value="1" /><input type="hidden" name="op" value="position_save" />'
		. '<br /><br /><input type="submit" value="' . _ADDPOSITION . '" />'
		. '</form></div>';
	CloseTable();
	echo '<br />';
	OpenTable();
	echo '<div style="text-align: center;"><b>' . _NOTE . '</b><br /><br />' . _POSITIONNOTE . '<br />' . _POSEXAMPLE . '</div>';
	CloseTable();
	include_once ('footer.php');
}
function position_save($apid = 0, $ad_position_number, $ad_position_name, $position_new = 0) {
	...
	if ($ad_position_name == '') {
		...
		echo '<br />';
		OpenTable();
		echo '<div style="text-align: center;" class="title">' . _ADDNEWPOSITION . '</div><br /><br />'
			. _POSINFOINCOMPLETE . '<br /><br />' . _GOBACK . '</span>';
		CloseTable();
		include_once('footer.php');
		die();
	}
	$ad_position_name = mysql_real_escape_string(check_html($ad_position_name, 'nohtml'));
...
function position_edit($apid) {
	...
	echo '<div style="text-align: center;" class="title">' . _EDITPOSITION . '<br /><br />'
		...
		. _POSITIONNUMBER . ': <b>' . $row['position_number'] . '</b><input type="hidden" name="ad_position_number" value="'
		. $row['position_number'] . '" /><input type="hidden" name="apid" value="'
		. $apid . '" /><input type="hidden" name="op" value="position_save" /><br /><br /><input type="submit" value="'
		. _SAVEPOSITION . '" />'
		. '</form></div>';
	CloseTable();
	include_once ('footer.php');
...
function position_delete($apid, $ok = 0, $active = 0, $new_pos = x) {
	...
	if ($numrows == 1) {
		...
		echo '<br />';
		OpenTable();
		echo '<div style="text-align: center;"><b>' . _DELETEPOSITION . '</b><br /><br />
	...
	if ($ok == 1) {
		...
		echo '<br /><div style="text-align: center;"><b>' . _DELETEPOSITION . ': ' . $row['position_name'] . "</b><br /><br />
		...
		if ($numrows != 0) {
			...
			echo '<div style="text-align: center;">[ <a href="' . $admin_file . '.php?op=ad_positions">' . _NO . '</a> | <a class="rn_csrf" href="'
				. $admin_file . '.php?op=position_delete&amp;apid=' . $apid . '&amp;ok=1">' . _YES . '</a> ]</div>';
		}
	}
	CloseTable();
	include_once('footer.php');
...
function ad_terms($save = 0, $terms_body = 0, $country = 0) {
	...
	if ($save != 0) {
		$terms_body = mysql_real_escape_string(check_html($terms_body, 'nocheck'));
	...
	echo '<div align="center"><span class="title">' . _EDITTERMS . '</span><br /><br /><i>' . _SITENAMEADS . '</i><br /><br />'
	...
	echo '</select><br />'
		...
		. '</form><br /><table border="0" width="80%" align="center"><tr><td align="center"><i>' . _TERMSNOTE . '</i></td></tr></table></div>';
	CloseTable();
	include_once ('footer.php');
}
function ad_plans() {
	...
	if ($numrows != 0) {
		...
		echo '<table border="1" width="100%"><tr><td bgcolor="' . $bgcolor2 . '"><b>&nbsp;'
			. _PLANNAME . '</b></td><td align="center" bgcolor="' . $bgcolor2 . '"><b>'
			. _DELIVERY . '</b></td><td align="center" bgcolor="' . $bgcolor2 . '"><b>'
			. _STATUS . '</b></td><td align="center" bgcolor="' . $bgcolor2 . '"><b>'
			. _PRICE . '</b></td><td align="center" bgcolor="' . $bgcolor2 . '"><b>'
			. _FUNCTIONS . '</b></td></tr>';
		while ($row = $db->sql_fetchrow($result)) {
	...
	echo '<tr><td>&nbsp;</td><td><input type="hidden" name="op" value="ad_plans_add" /><input type="submit" value="'
		. _ADDNEWPLAN . '" /></td></tr></table></form><br /><div style="text-align: center;"><i>' . _PLANSNOTE . '</i></div>';
	CloseTable();
	include_once ('footer.php');
}
function ad_plans_add($name, $description, $delivery, $type, $price, $buy_links, $status) {
	...
	if (!empty($name) AND !empty($description) AND !empty($delivery) AND (isset($type) AND is_numeric($type)) AND !empty($price) AND !empty($buy_links) AND !empty($status)) {
		$name = mysql_real_escape_string(check_html($name, 'nohtml'));
		$description = mysql_real_escape_string(check_html($description, 'nocheck'));
		$price = mysql_real_escape_string(check_html($price, 'nohtml'));
		$buy_links = mysql_real_escape_string(check_html($buy_links, 'nocheck'));
...
function ad_plans_edit($pid) {
	...
	echo '<tr><td>&nbsp;</td><td><input type="hidden" name="pid" value="' . $pid . '" />'
		...
		. '<br /><div style="text-align: center;"><i>' . _PLANSNOTE . '</i></div>';
	CloseTable();
	include_once ('footer.php');
}
...
function ad_plans_delete($pid, $ok = 0) {
	...
	if ($ok == 1) {
		...
		OpenTable();
		$row = $db->sql_fetchrow($db->sql_query('SELECT * FROM ' . $prefix . '_banner_plans WHERE pid=\'' . $pid . '\''));
		echo '<div style="text-align: center;"><b>' . _DELETEPLAN . ': ' . $row['name'] . '</b><br /><br />'
