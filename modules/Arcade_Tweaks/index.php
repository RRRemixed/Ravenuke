 ...
 */ 
// accessing this file directly is a nono...
if (!eregi("modules.php", $_SERVER['PHP_SELF'])) {
    die ("You can't access this file directly...");
} 
// The following paths are my "standart one"... check that they're yours also...
$index = 0; // $index=1 we want the right blocks, $index=0 we dont want the right blocks
...
// --------------------------------------------
// Carefull with edition above this line......
// --------------------------------------------
...
function ViewPictures()
{
    ...
    include("header.php");
    menu();
    title("Pictures in <b>$picdirectory</b>");
    OpenTable();
...
function MakeIntelliAddForm($basename)
{
    ...
    $content .= "	title: <input type=\"text\" name=\"gametitle\" size=\"12\" value=\"$basename\">\n";
    ...
    $content .= "	</select>\n";
    $content .= "	<input type=\"submit\" value=\"IntelliAdd\">";
    $content .= "</form>\n";
    return $content;
} 
function checkSwfDir()
{
    ...
    // the end of the page... :/
    // BTW, I dont really understand why it's a problem PhantomK :?
    $content = " <tr>\n"
    ...
    for ($i = $cstart;$i <= $cend;$i++) {
        ...
        if (isset($row["game_pic"]) and $row["game_pic"] != "") {
        ...
        switch ($count) {
            ...
            case 1 :
                ...
                $content .= "<td><b>" . $row["game_name"] . "</b>: type " . $row["game_type"] . ", " . $row["game_width"] . "x" . $row["game_height"] . "<br>"
            ...
            default:
                $swfMultipleInDB++;
                $content .= "<td><big><b><u>Multiple entry in DB</u></b></big></td>\n";
                break;
        } 
        $content .= "</tr>\n";
        $db->sql_freeresult();
    ...
    echo "	<tr>\n";
    echo "     <td colspan=3 border=0 valign=top ><b><u>Results</u></b>:<br><ul><li><b>" . count($swf_array) . "</b> total .SWF files in directory<li><b>" . $totalcount[0] . "</b> in database"
     . "<li><b>" . (count($swf_array) - $totalcount[0]) . "</b> unreferenced in database</ul></td>\n";
    echo "		<td colspan=2><b><u>On this page</u></b><ul><li>$swfCnt Flash Files<li>" . ($swfOK + $swfMultipleInDB) . " referenced in DB<li>$swfMultipleInDB referenced more than once<li>$swfOK referenced once.<li>$swfNotInDB not referenced</ul></td>\n";
    echo "	</tr>\n"; 
...
function EditComment()
{
	...
	if ($row) {
		...
		echo "<tr><td></td><td width=150 align=center><b>Highscore: </b> ".$row["game_highscore"]
		...
		echo "<tr><td></td><td></td><td align=\"center\" colspan=\"10\"><input type=\"submit\" value=\"Change Comment\"></td></tr>\n";
		echo "</form>\n";
		echo "</table>\n";
	}
    CloseTable();
...
function SearchDBForm()
{
    ...
    $content .= "<b>Search:<b>&nbsp;<input type=\"text\" name=\"srchstring\" value=\"$srchstring\" size=\"10\" title=\"Use Wildcards * and ? such as 'yeti*'\">\n";
    $content .= "&nbsp;<b>in</b>&nbsp;<select name=\"searchin\">\n";
...
<?php
    ...
    echo "<tr><td colspan=5 width='50%'><b><u>Results</u></b>:<ul>";
    ...
    if ($gameswithoutpic > 0) {
        echo "<li>$gameswithoutpic games dont have a pic";
    } else
...
function SubmitChange()
{
    ...
    if (!is_array($gameid) or count($gameid) == 0 or !isset($selectaction) or $selectaction == "") {
    ...
    switch ($selectaction) {
        case "changecat":
            ...
            echo "</select>&nbsp;<input type=\"submit\" name=\"Move\">";
        ...
        case "changesize":
            ...
            echo "</ul>";
            echo "<u>New window size</u>:  <b>Width:</b> <input type=\"text\" name=\"gwidth\" value=\"\" size=\"4\">";
            echo "   <b>Height:</b> <input type=\"text\" name=\"gheight\" value=\"\" size=\"4\">";
            echo "&nbsp;<input type=\"submit\" value=\"Set Size\">";
        ...
        case "removescore":
            ...
            echo "<br><input type=\"submit\" value=\"I Confirm\"></form>";
            echo "<form action=\"modules.php?name=$module_name&amp;m_op=checkDB\" name=\"dontconfirm\" method=\"post\">";
            echo "<input type=\"submit\" value=\"No,Dont Remove scores\"></form>";
            CloseTable();
            include("footer.php");
            break;
        case "removefromdb":
            ...
            echo "</ul>";
            echo "Please Confirm you want to <b><big>remove this " . count($gameid) . " game(s) from DB</big></b> (Files will stay in your games directory.): ";
            echo "<input type=\"hidden\" name=\"selectaction\" value=\"removefromdb\">";
            echo "<br><input type=\"submit\" value=\"I Confirm\"></form>";
            echo "<form action=\"modules.php?name=$module_name&amp;m_op=checkDB\" name=\"dontconfirm\" method=\"post\">";
            echo "<input type=\"submit\" value=\"No,Dont Remove this games\"></form>";
            CloseTable();
            include("footer.php");
            break;
        case "removefiles":
...
function ApplyChange()
{
    ...
    switch ($selectaction) {
        case "changecat":
            ...
            foreach($_categoryArray as $row) {
                if ($row["arcade_catid"] == $catid) {
                    echo "have been moved to category <b>" . $row["arcade_cattitle"] . "</b><br>";
                    break;
                } 
        ...
        case "changesize":
            ...
            echo "</ul>";
            echo " ... to <b>width:</b> $gwidth , <b>height:</b> $gheight";
            CloseTable();
            include("footer.php");
            break;
        case "removescore":
...
function MakePageDBLinks($start, $end, $delta, $currentpos = null)
{
    global $module_name, $type012;
    $content = "<font class='content'><b>Goto Pages:&nbsp;&nbsp;</b>";
    $loccount = $start;
    $page = 1;
    ...
    $content .= "<a href='modules.php?name=$module_name&amp;m_op=checkDB&amp;cstart=$start&amp;cend=$end'> ALL</a>";
    $content .= "</font>";
    echo $content;
} 
function MakePageLinks($start, $end, $delta, $currentpos = null)
{
    ...
    $content .= "<a href='modules.php?name=$module_name&amp;m_op=checkswf$addfilter&amp;cstart=$start&amp;cend=$end'> ALL</a>";
    $content .= "</font>";
    return $content;
} 
...
function ShowCategory()
{
    ...
    echo "<input type=\"text\" name=\"title\" value=\"\" length=\"15\" maxlength=\"100\">&nbsp;";
...
function FixDB()
{
    ...
    if ($fix = 'commentstable') {
        ...
        if ($commenttableexist) {
            echo "<li><b>Comments Module</b> found.<br>Checking if all games have their comments in comments tables.<br><br>";
            ...
            while ($row = $db->sql_fetchrow($result)) {
                if (!$db->sql_numrows($db->sql_query("SELECT * FROM " . $prefix . "_bbarcade_comments WHERE `game_id`=" . $row["game_id"]))) {
                    ...
                    echo "Comments Added for <b>" . $row["game_name"] . "</b><br>";
                } 
            } 
        } else {
            "Cannot fix your <b>Comments Table</b>, I dont find the table in database.";
        } 
    } 
...
function DBCoherence()
{
    ...
    echo "<li> <b>Category Checking</b><br><img src='modules/$module_name/images/icon_query.gif' border=0 width=16 height=16> Check if the number of games in category table match the count of games<br><br>";
    CategoryTable(0); // read the category table;
    global $_categoryArray;
    ...
    foreach($_categoryArray as $row) {
        ...
        $count = $db->sql_fetchrow($result);
        if ($count["count"] <> $row["arcade_nbelmt"]) {
            echo "<b>" . $row["arcade_cattitle"] . "</b> not synchronized. [ Not Matching - <a href=\"modules.php?name=$module_name&amp;m_op=resynccat&amp;catid=" . $row["arcade_catid"] . "\">Re-Sync</a> ]<br>";
            $CategoryNotSynchronized++;
        } 
    } 
    if ($CategoryNotSynchronized) {
        echo "<img src='modules/$module_name/images/icon_warn.gif' border=0 width=16 height=16><b>$CategoryNotSynchronized</b> dont match. [ <a href='modules.php?name=$module_name&m_op=syncallcat'>Resync All Categories</a> ]";
    } else {
        echo "<img src='modules/$module_name/images/icon_info.gif' border=0 width=16 height=16><b>All categories are synchronized.</b>";
    } 
    echo "<br><br>"; 
    ...
    if ($commenttableexist) {
        echo "<li><b>Comments Module</b> found.<br><img src='modules/$module_name/images/icon_query.gif' border=0 width=16 height=16> Checking if all games have their comments in comments tables.<br><br>";
        $sql = "SELECT * FROM " . $prefix . "_bbgames ";
        if (!$result = $db->sql_query($sql)) {
            die("Cannot Open Games Table");
        } 
        ...
        echo $missingcomment? "<br><br><img src='modules/$module_name/images/icon_warn.gif' border=0 width=16 height=16><b>" . $missingcomment . " games dont have comments</b> [ <a href='modules.php?name=$module_name&amp;m_op=fixdb&amp;fix=commentstable'>Fix Comments Table</a> ]<br>"
        : "<img src='modules/$module_name/images/icon_info.gif' border=0 width=16 height=16><b>All games have their comments rows.</b><br>";
    } else
        echo "<li><img src='modules/$module_name/images/icon_info.gif' border=0 width=16 height=16> <b>Comments Module</b> not found.";
...
function ShowDefault()
{
    ...
    echo "<b>Thanks to test Arcade Tweaks RC2.</b><br>";
    echo "Note that this module is <u>still</u> on Test Mode."
		 ."<br>It have been tested succesfully on Nuke 6.8 and 7.3 with Arcade Mod V3."
    ...
    echo "<br><br>";
    echo "Please , Come to <a href=\"http://www.portedmods.com/\">http://www.portedmods.com</a> or <a href=\"http://www.thehorde.be/modules.php?name=Downloads&d_op=viewdownload&cid=5\">http://www.thehorde.be</a> for update and comments.";
    echo "<br>Rica<br><font class='content'><ul>\n";
    echo "<li><a href='modules.php?name=$module_name&m_op=checkswf'><b>All SWF</b></a>:<br>\n";
    ...
    echo "<br><li><a href='modules.php?name=$module_name&m_op=checkswf&amp;filefilter=unreferenced'><b>Unreferenced SWF</b></a>:<br>\n";
    echo "Same than above but only show unreferenced SWF";
    echo "<br><li><a href='modules.php?name=$module_name&m_op=checkDB'><b>Check DB</b></a>:<br>\n";
    ...
    echo "<br><li><a href='modules.php?name=$module_name&m_op=category'><b>Category</b></a>:<br>\n";
    echo "Add , resync and check games categories";
    echo "<br><li><a href='modules.php?name=$module_name&m_op=tools'><b>Tools</b></a>:<br>\n";
