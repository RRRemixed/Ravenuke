...
if( !file_exists($phpbb_root_path . 'language/lang_' . $board_config['default_lang'] . '/lang_jail.' . $phpEx)) 
{ 
	$board_config['default_lang'] = 'english'; 
} 
include_once($phpbb_root_path . 'language/lang_' . $board_config['default_lang'] . '/lang_jail.' . $phpEx);
...
function cell_create_time($cell_time)
{
	...
	if ( $days > 1 )
	{
		$time_remaining .= $days.'&nbsp;'.$lang['Cell_days'].'&nbsp;';
	}
	else if ( $days != 0 )
	{
		$time_remaining .= $days.'&nbsp;'.$lang['Cell_day'].'&nbsp;';
	}
	if ( $hours > 1 )
	{
		$time_remaining .= $hours.'&nbsp;'.$lang['Cell_hours'].'&nbsp;';
	}
	else if ( $hours != 0 )
	{
		$time_remaining .= $hours.'&nbsp;'.$lang['Cell_hour'].'&nbsp;';
	}
	if ( $minutes > 1 )
	{
		$time_remaining .= $minutes.'&nbsp;'.$lang['Cell_minutes'].'&nbsp;';
	}
	else if ( $minutes != 0 )
	{
		$time_remaining .= $minutes.'&nbsp;'.$lang['Cell_minute'].'&nbsp;';
	}
...
function cell_imprison_user($celled_id , $time_day , $time_hour , $time_minute , $caution , $cautionable , $freeable , $sentence , $punishment )
{
	...
	$sql = "INSERT INTO " . JAIL_USERS_TABLE . "
