...
function append_sid($url, $non_html_amp = false)
{
   ...
   $amp = $non_html_amp ? '&' : '&amp;';
	...
	} elseif (ereg("attach_rules", $url) OR ereg("download.php",$url) OR ereg("uacp.php",$url) OR ereg("viewtopic.php",$url)) {
   ...
   if ($userdata['user_level'] > 1) {
	if ( !empty($SID) && !eregi('sid=', $url) && isset($_COOKIE)) // montego - added isset() check to try and keep SE Bots from crawling the sid
	{
