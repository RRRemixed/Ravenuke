        ...
        $mode == 'register' )
{
	...
	$strip_var_list = array('email' => 'email', 'website' => 'website',  'fb' => 'fb', 'tw' => 'tw', 'skype' => 'skype', 'steam' => 'steam', 'location' => 'location', 'occupation' => 'occupation', 'interests' => 'interests', 'confirm_code' => 'confirm_code', 'birthday' => 'birthday');
	...
	$signature_bbcode_uid = '';
        ...
        validate_optional_fields($website, $fb, $tw, $skype, $steam, $location, $occupation, $interests, $signature);
      ...
      { 
		...
		if ( !$error ) {
			...
			if ( $mode == 'editprofile' ) {
				...
				$sql = "UPDATE " . USERS_TABLE . "
				SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_from_flag = '$user_flag', user_interests = '" . str_replace("\'", "''", $interests) . "', user_viewemail = $viewemail, user_fb = '" . str_replace("\'", "''", $fb) . "', user_tw = '" . str_replace("\'", "''", $tw) . "', user_skype = '" . str_replace("\'", "''", $skype) . "', user_steam = '" . str_replace("\'", "''", $steam) . "', user_attachsig = $attachsig, user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_notify_pm = $notifypm, user_popup_pm = $popup_pm, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_active = $user_active, user_actkey = '" . str_replace("\'", "''", $user_actkey) . "'" . $avatar_sql . ", user_gender = $gender, user_birthday = $birthday, user_next_birthday_greeting = $next_birthday_greeting
					WHERE user_id = $user_id";
				...
				message_die(GENERAL_MESSAGE, $message);
                        ...
                        //
                        $reg_date = date("d M Y");
			$sql = "INSERT INTO " . USERS_TABLE . "	(user_id, username, user_regdate, user_password, user_email, user_website, user_occ, user_from, user_from_flag, user_interests, user_sig, user_sig_bbcode_uid, user_avatar, user_avatar_type, user_viewemail, user_fb, user_tw, user_skype, user_steam, user_attachsig, user_allowsmile, user_allowhtml, user_allowbbcode, user_allow_viewonline, user_notify, user_notify_pm, user_popup_pm, user_timezone, user_dateformat, user_lang, user_style, user_gender, user_birthday, user_next_birthday_greeting, user_level, user_allow_pm, user_active, user_actkey)
                            VALUES ($user_id, '" . str_replace("\'", "''", $username) . "', '" . addslashes($reg_date) . "', '" . str_replace("\'", "''", $new_password) . "', '" . str_replace("\'", "''", $email) . "', '" . str_replace("\'", "''", $website) . "', '" . str_replace("\'", "''", $occupation) . "', '" . str_replace("\'", "''", $location) . "', '$user_flag', '" . str_replace("\'", "''", $interests) . "', '" . str_replace("\'", "''", $signature) . "', '$signature_bbcode_uid', $avatar_sql, $viewemail, '" . str_replace("\'", "''", $fb) . "', '" . str_replace("\'", "''", $tw) . "', '" . str_replace("\'", "''", $skype) . "', '" . str_replace("\'", "''", $steam) . "', $attachsig, $allowsmilies, $allowhtml, $allowbbcode, $allowviewonline, $notifyreply, $notifypm, $popup_pm, $user_timezone, '" . str_replace("\'", "''", $user_dateformat) . "', '" . str_replace("\'", "''", $user_lang) . "', $user_style, '$gender', '$birthday' ,'$next_birthday_greeting', 0, 1, ";
...
if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
{
	...
	$avatar_category = ( !empty($HTTP_POST_VARS['avatarcategory']) ) ? htmlspecialchars($HTTP_POST_VARS['avatarcategory']) : '';
        ...
        display_avatar_gallery($mode, $avatar_category, $user_id, $email, $current_email, $coppa, $username, $email, $new_password, $cur_password, $password_confirm, $website,  $fb, $tw, $skype, $steam, $location, $user_flag, $occupation, $interests, $signature, $viewemail, $notifypm, $popup_pm, $notifyreply, $attachsig, $allowhtml, $allowbbcode, $allowsmilies, $allowviewonline, $user_style, $user_lang, $user_timezone, $user_dateformat, $userdata['session_id'], $gender, $birthday);
}
else
{
...
// FLAGHACK-end
		...
		//signature editor
                // Start add - Gender MOD
                'LOCK_GENDER' =>($mode!='register') ? 'ENABLED':'', 
                'GENDER' => $gender, 
