...
if( ( $total_categories = count($category_rows) ) )
{
        ...
        {
                case 'postgresql':
                        $sql = "SELECT f.*, p.post_time, p.post_username, u.username, u.user_id, u.user_color_gc
...
$db->sql_freeresult($result);
	...
	while(list($config_name, $config_value) = $db->sql_fetchrow($resulta)){
		$aconf[$config_name] = $config_value;
	}
        $showmenu = $aconf['showmenu'];
        if ($showmenu == 0 AND is_user($user)) {
        include("modules/$module_name/includes/arcade_menu.php");
        }
        include("modules/$module_name/includes/fmessagebox.php");
