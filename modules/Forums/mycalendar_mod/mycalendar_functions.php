...
function mycal_forum_check($forum_id) 
{
    ...
    if (!isset($events_forums)) {
        ...
        if (!$results = $db->sql_query($sql)) {
            message_die(GENERAL_ERROR, 'Failure when checking if forum can hold events.');
        }
...
function mycal_submit_event($mode, $forum_id, $topic_id, $post_id)
{
    ...
    if (!empty($HTTP_POST_VARS['cal_interval_date']) && (isset($HTTP_POST_VARS['cal_date_end']) || !empty($HTTP_POST_VARS['cal_repeat_always']))) {
        ...
        elseif ($HTTP_POST_VARS['cal_date_end'] != $lang['Date_not_specified']) {
            ...
            $sql .= ' as cal_repeat';
            if (!$results = $db->sql_query($sql)) {
                message_die(GENERAL_MESSAGE, 'Could not determine repeat multiplier for date entry.');
            }
    ...
    if ($mode == 'newtopic' && $cal_isodate && mycal_forum_check($forum_id)) {
        ...
        if (!$db->sql_query($sql)) {
            message_die(GENERAL_MESSAGE, 'Could not insert new event into calendar table.');
        }
    } 
    ...
    elseif ($mode == 'editpost' && mycal_forum_check($forum_id)) {
        ...
        if (!$result = $db->sql_query($sql)) {
            message_die(GENERAL_ERROR, 'Failure when looking up previous date entry for editing topic.');
        }
            ...
            if (!$cal_isodate) {
                $sql = 'DELETE FROM ' .
                       ...
                       'WHERE ' .
                           "topic_id = $topic_id";
                if (!$db->sql_query($sql)) {
                    message_die(GENERAL_MESSAGE, 'Could not remove event from calendar table.');
                }
            }
            else {
                $sql = 'UPDATE ' .
                       ...
                       'WHERE ' .
                           "topic_id = $topic_id";
                if (!$db->sql_query($sql)) {
                    message_die(GENERAL_MESSAGE, 'Could not update event in calendar table.');
                }
            }
        }
...
function mycal_move_event($new_forum_id, $topic_id, $leave_shadow = false)
{
    ...
    if (!$leave_shadow && !mycal_forum_check($new_forum_id)) {
        ...
        if (!$db->sql_query($sql)) {
            message_die(GENERAL_MESSAGE, 'Could not relocate the event.'); 
        }
    }
}
...
function mycal_delete_event($topic_id, $post_id, $check_post = false) 
{
    ...
    if (!$check_post || $db->sql_numrows($result) > 0) {
        ...
        if (!$db->sql_query($sql)) {
            message_die(GENERAL_MESSAGE, 'Could not delete event.'); 
        }
    }
}
...
function mycal_show_event($topic_id, $post_id, $message) 
{
    ...
    if ( $db->sql_numrows($result) > 0 ) {
        ...
        $cal_repeat = $row['cal_repeat'];
        $event['message'] = '<b>' . $lang['Calendar_event_title'] . ':</b> <i>' . mycal_translate_date($cal_date_f) . '</i>';
        ...
        if ($row['cal_repeat'] != 1) {
            ...
            if ($row['cal_repeat'] > 1) {
                ...
                if (!$results2 = $db->sql_query($sql2)) {
                    message_die(GENERAL_ERROR, 'Failure when calculating end date.');
                }
            ...
            if ($row['cal_repeat'] > 1) {
                ...
                $sql2 = 'SELECT ' .
                         ...
                         'WHERE ' .
                             'topic_id = ' . $topic_id;
                if (!$results2 = $db->sql_query($sql2)) {
                    message_die(GENERAL_ERROR, 'Failure when looking up end date for previous date entry on topic');
                }
...
function mycal_first_post($topic_id, $post_id)
{
    ...
    $sql = 'SELECT ' .
           ...
           'WHERE ' .
               "topic_id = $topic_id"; 
    if (!$results = $db->sql_query($sql)) {
        message_die(GENERAL_ERROR, 'Failure when determining if post is leading post in topic.');
    }
