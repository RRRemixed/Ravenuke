...
include("modules/Forums/mycalendar_mod/mycalendar_functions.php");
...
//
$sql = "SELECT u.*, u.user_allow_viewonline, u.user_session_time, p.*, pt.post_text, pt.post_subject, pt.bbcode_uid
...
// FLAGHACK-end
	...
	default : $gender_image=""; 
                ...
                $fb_img = ( $postrow[$i]['user_fb'] ) ? '<a href="' . $postrow[$i]['user_fb'] . '" target="_blank"><img src="' . $images['icon_fb'] . '" alt="' . $lang['Visit_fb'] . '" title="' . $lang['Visit_fb'] . '" border="0" /></a>' : '';
                $fb = ( $postrow[$i]['user_fb'] ) ? '<a href="' . $postrow[$i]['user_fb'] . '" target="_blank">' . $lang['Visit_fb'] . '</a>' : '';

                $tw_img = ( $postrow[$i]['user_tw'] ) ? '<a href="' . $postrow[$i]['user_tw'] . '" target="_blank"><img src="' . $images['icon_tw'] . '" alt="' . $lang['Visit_tw'] . '" title="' . $lang['Visit_tw'] . '" border="0" /></a>' : '';
                $tw = ( $postrow[$i]['user_tw'] ) ? '<a href="' . $postrow[$i]['user_tw'] . '" target="_blank">' . $lang['Visit_tw'] . '</a>' : '';

                $skype_img = ( $postrow[$i]['user_skype'] ) ? '<a href="' . $postrow[$i]['user_skype'] . '" target="_blank"><img src="' . $images['icon_skype'] . '" alt="' . $lang['Visit_skype'] . '" title="' . $lang['Visit_skype'] . '" border="0" /></a>' : '';
                $skype = ( $postrow[$i]['user_skype'] ) ? '<a href="' . $postrow[$i]['user_skype'] . '" target="_blank">' . $lang['Visit_skype'] . '</a>' : '';

                $steam_img = ( $postrow[$i]['user_steam'] ) ? '<a href="' . $postrow[$i]['user_steam'] . '" target="_blank"><img src="' . $images['icon_steam'] . '" alt="' . $lang['Visit_steam'] . '" title="' . $lang['Visit_steam'] . '" border="0" /></a>' : '';
                $steam = ( $postrow[$i]['user_steam'] ) ? '<a href="' . $postrow[$i]['user_steam'] . '" target="_blank">' . $lang['Visit_steam'] . '</a>' : '';
        }
        else
        {
                $profile_img = '';
    ...
    if ($bbcode_uid != '')
    {
        ...
        if($postrow[$i]['user_allow_viewonline']){
           $status = '<a href="modules.php?name=Forums&file=viewonline">&nbsp;<img src="' . $images['icon_online'] . '" border="0" /></a>';
           }else{
              $status = '&nbsp;<img src="' . $images['icon_offline'] . '" border="0" /></a>';
              }
           }else{
              $status = '&nbsp;<img src="' . $images['icon_offline'] . '" border="0" /></a>';
                }
		$isclanmem = $db->sql_query("select username,clan_member from ".$prefix."_users where username='".$poster."' and clan_member='1'");
		$countmem = $db->sql_numrows($isclanmem);
		$isclanmem2 = $db->sql_query("select username,clan_member,cmod_alias,cmod_country from ".$prefix."_users where username='$poster'");
		$rowcmod = $db->sql_fetchrow($isclanmem2);
		$c_country = ($countmem == "1") ? "Country :&nbsp;&nbsp;" : "";
		$c_alias = ($countmem == "1") ? "Alias :&nbsp;&nbsp;" : "";
		$country = ($countmem == "1") ? $rowcmod['cmod_country'] : "";
		$u_alias = ($countmem == "1") ? $rowcmod['cmod_alias'] : "";
		$u_country = ($countmem == "1") ? "<img src=\"images/roster/$country.gif\">" : "";
