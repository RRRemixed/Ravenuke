...
if ($mode == "main" || !$mode)
	...
	$groups_info = $db->sql_fetchrowset($r);
		for ($a = 0; $a < count($groups_info); $a++)
			{
			if (eregi('--'. $groups_info[$a]['group_id'] .'--', $new_id))
				{
	...
	message_die(GENERAL_MESSAGE, $lang['group_delete_user_2'], $lang['success']);
		}
	...
	$color 	= $HTTP_POST_VARS['color'];
		...
		if ($multi)
			$who = '';
		if ( (!$who) && (!$multi) )
			message_die(GENERAL_ERROR, $lang['choose_user_id_error']);
	...
	message_die(GENERAL_MESSGAE, $lang['group_user_added'], $lang['success']);
		}
