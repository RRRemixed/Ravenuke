...
if ( $mode == 'edit' || $mode == 'save' && ( isset($HTTP_POST_VARS['username']) || isset($HTTP_GET_VARS[POST_USERS_URL]) || isset( $HTTP_POST_VARS[POST_USERS_URL]) ) )
{
		...
		$username = ( !empty($HTTP_POST_VARS['username']) ) ? phpbb_clean_username($HTTP_POST_VARS['username']) : '';
                ...
                $tw = ( !empty($HTTP_POST_VARS['tw']) ) ? trim(strip_tags( $HTTP_POST_VARS['tw'] ) ) : '';
                $skype = ( !empty($HTTP_POST_VARS['skype']) ) ? trim(strip_tags( $HTTP_POST_VARS['skype'] ) ) : '';
                $steam = ( !empty($HTTP_POST_VARS['steam']) ) ? trim(strip_tags( $HTTP_POST_VARS['steam'] ) ) : '';
...
// FLAGHACK-end
                ...
                if( isset( $HTTP_POST_VARS['avatargallery'] ) || isset( $HTTP_POST_VARS['submitavatar'] ) || isset( $HTTP_POST_VARS['cancelavatar'] ) )
                {
                        ...
                        $tw = htmlspecialchars(stripslashes($tw));
                        $skype = htmlspecialchars(stripslashes($skype));
                        $steam = htmlspecialchars(stripslashes($steam));
        ...
        if( isset( $HTTP_POST_VARS['submit'] ) )
        {
                ...
                if( !$error )
                {
                        $sql = "UPDATE " . USERS_TABLE . "
                                SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_from_flag = '$user_flag', user_interests = '" . str_replace("\'", "''", $interests) . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_viewemail = $viewemail, user_fb = '" . str_replace("\'", "''", $fb) . "', user_tw = '" . str_replace("\'", "''", $tw) . "', user_skype = '" . str_replace("\'", "''", $skype) . "', user_steam = '" . str_replace("\'", "''", $steam) . "', user_attachsig = $attachsig, user_sig_bbcode_uid = '$signature_bbcode_uid', user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowavatar = $user_allowavatar, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_allow_pm = $user_allowpm, user_notify_pm = $notifypm, user_popup_pm = $popuppm, user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_active = $user_status, user_rank = $user_rank" . $avatar_sql . ", user_gender = '$gender, user_birthday = $birthday, user_next_birthday_greeting=$next_birthday_greeting'
				WHERE user_id = $user_id";
...
// FLAGHACK-end
        ...
        else if( !isset( $HTTP_POST_VARS['submit'] ) && $mode != 'save' && !isset( $HTTP_POST_VARS['avatargallery'] ) && !isset( $HTTP_POST_VARS['submitavatar'] ) && !isset( $HTTP_POST_VARS['cancelavatar'] ) )
        {
                ...
                $tw = htmlspecialchars($this_userdata['user_tw']);
                $skype = htmlspecialchars($this_userdata['user_skype']);
                $steam = htmlspecialchars($this_userdata['user_steam']);
...
// FLAGHACK-end
        ...
        if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
        {
                if( !$error )
                {
                        ...
                        $s_hidden_fields .= '<input type="hidden" name="tw" value="' . str_replace("\"", "&quot;", $tw) . '" />';
                        $s_hidden_fields .= '<input type="hidden" name="skype" value="' . str_replace("\"", "&quot;", $skype) . '" />';
                        $s_hidden_fields .= '<input type="hidden" name="steam" value="' . str_replace("\"", "&quot;", $steam) . '" />';
