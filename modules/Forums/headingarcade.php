...
$template->set_filenames(array(
   'headingarcade' => 'headingarcade_body.tpl')
	...
	$template->assign_block_vars('arcaderow3',array(  
        ...
        $color_name = CheckUsernameColor($userdata['user_color_gc'], $userdata['username']);
...
$sql = "SELECT COUNT(*) AS nbvictoires, g.game_highuser, u.user_id, u.username, u.user_color_gc, u.user_level FROM " . GAMES_TABLE . " g, " . USERS_TABLE . " u WHERE g.game_highuser = u.user_id AND g.game_highuser<>0 GROUP BY g.game_highuser ORDER BY nbvictoires DESC LIMIT 0,10  ";
...
// Start of Last 5 highscores 
   $sql = " SELECT g.* , u.username, u.user_color_gc FROM " . GAMES_TABLE . " g, " . USERS_TABLE . " u WHERE g.game_highuser = u.user_id ORDER BY game_highdate DESC LIMIT 0,5  ";
   if ( !($result = $db->sql_query($sql)) ) 
   { 
   ...
   {
   	$rowArcade['username'] = CheckUsernameColor($rowArcade['user_color_gc'], $rowArcade['username']);
	$class = ($class == 'row1') ? 'row2' : 'row1' ;
...
// Last Recorded Score
   $sql = " SELECT u.user_id, u.username, u.user_color_gc, s.game_id, s.score_game, s.score_date, g.game_name FROM " . SCORES_TABLE . " s, " . USERS_TABLE . " u, " . GAMES_TABLE . " g WHERE s.user_id = u.user_id AND s.game_id = g.game_id ORDER BY score_date DESC LIMIT 0,1 ";
   if ( !($result = $db->sql_query($sql)) )
   {
   ...
   {
      $rowScore['username'] = CheckUsernameColor($rowScore['user_color_gc'], $rowScore['username']);
...
IF ( empty($avatar_img) ) 
{ 
   $avatar_img = '<img src="' . $board_config['avatar_gallery_path'] . '/blank.gif" alt="Default Avatar" border="0" />'; 
}
// Finished avatar
