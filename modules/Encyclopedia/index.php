...
function alpha($eid) {
    ...
    echo '<center><p><b>'._ENCYSELECTLETTER.'</b></p>';
...
function terms($eid, $ltr) {
    ...
    if (($active == 1) OR (is_admin($admin))) {
        if (($active != 1) AND (is_admin($admin))) {
            echo '<center><p>'._YOURADMINENCY.'</p></center>';
        }
        echo '<center><p class="title">' . _EM_LISTOPTION . '</p></center>';
        ...
        if ($numrows == 0) {
            echo '<center><p><i>'._NOCONTENTFORLETTER.' '.htmlentities($ltr).'.</i></p></center>';
        } else {
        ...
        echo '<center><p>'._ENCYNOTACTIVE.'</p><p>'
            ._GOBACK.'</p></center>';
    }
    CloseTable();
    include_once('footer.php');
...
function list_themes() {
    ...
    if (is_admin($admin)) {
        ...
        echo "<p class=\"content\" align=\"center\"><b>"._YOURADMININACTIVELIST."</b></p>";
