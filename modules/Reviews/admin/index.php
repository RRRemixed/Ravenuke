...
function reviews() {
	...
	GraphicAdmin();
	OpenTable();
	echo '<center><span class="title">' . _REVADMIN . '</span></center>';
	CloseTable();
	...
	if ($numrows > 0) {
		while (list($id, $date, $title, $text, $reviewer, $email, $score, $url, $url_title, $rlanguage) = $db->sql_fetchrow($result)) {
			...
			echo '<form action="' . $admin_file . '.php" method="post">'
				...
				. '<tr><td>' . _PRODUCTTITLE . ':</td><td><input type="text" name="title" value="' . htmlspecialchars($title) . '" size="50" maxlength="60" /></td></tr>';
			if ($multilingual == 1) {
			...
			wysiwyg_textarea('text', $text, 'PHPNukeAdmin', '60', '10');
			echo '</td></tr>'
			...
			echo '<tr><td>' . _IMAGE . ':</td><td><input type="text" name="cover" size="41" maxlength="100" /><br /><i>' . _REVIMGINFO . '</i></td></tr></table>';
	...
	CloseTable();
	OpenTable();
	echo '<center><span class="option"><b>' . _DELMODREVIEW . '</b></span><br /><br />'
		. _MODREVINFO . '</center>';
	CloseTable();
	include_once('footer.php');
}
