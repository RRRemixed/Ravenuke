...
if ($op == 'nfSaveSubscript') 
{
  ...
  if ($error == '') {
    $name     = strip_tags(html_entity_decode($name, ENT_QUOTES));
    $tagline  = strip_tags(html_entity_decode($tagline, ENT_QUOTES));
    $image    = strip_tags(html_entity_decode($image, ENT_QUOTES));
    $icon     = strip_tags(html_entity_decode($icon, ENT_QUOTES));
    $url      = strip_tags(html_entity_decode($url, ENT_QUOTES));
