...
if (get_magic_quotes_gpc()) {
	...
	$gsubject = stripslashes($gsubject);
}
...
if($mailsuccess) {
	...
	include('header.php');
	Opentable();
	echo '<center><h3>'._GR_GROUPSEMAIL.'</h3>'.PHP_EOL;
	echo _GR_MAILSENTGR.'<br />'._GR_WAITREDIR.'</center>'.PHP_EOL;
	CloseTable();
	include('footer.php');
} else {
	include('header.php');
	Opentable();
	echo '<center><h3>'._GR_GROUPSEMAIL.'</h3>'.PHP_EOL;
	echo _GR_MAILSENDERROR.'<br /><br />'._GOBACK.'</center>'.PHP_EOL;
	CloseTable();
	include('footer.php');
}
?>