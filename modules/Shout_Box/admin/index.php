...
global $prefix, $user_prefix, $db, $admin_file, $module_name;
...
$SHadminlangFile = 'modules/'.$module_name.'/admin/language/lang-'.$currentlang.'.php';
if (file_exists($SHadminlangFile))
{
   include_once $SHadminlangFile;
}
else
{
...
$SHlangFile = 'modules/'.$module_name.'/language/lang-'.$currentlang.'.php';
if (!file_exists($SHlangFile))
{
   $SHlangFile = 'modules/'.$module_name.'/language/lang-english.php';
}
...
function ShoutBoxAdminMenu($ShoutMenuOptionActive) {
	global $prefix, $db, $admin_file, $rowColor;
	OpenTable();
	...
	CloseTable();
	// Added file check as original seems to take forever
if (file_exists('SB_SQL_installer.php'))
{
   OpenTable();
   echo '<div align="center">' . "\n";
   echo ''._SHOUTINSTALLEXISTS.'';
   echo '</div>' . "\n";
   CloseTable();
}
	echo "<br />";
...
function manageShouts($page, $pruned) {
	global $prefix, $user_prefix, $db, $admin, $admin_file, $module_name, $sbURL, $user, $cookie;
	...
	echo "<td><span class=\"content\"><strong>"._SB_SHOUT."</strong></span></td>";
	...
	while ($shout = $db->sql_fetchrow($nameresult)) {
		...
		$sql = "SELECT * FROM ".$user_prefix."_users where username='$shout[name]'";
		$nameresult2 = $db->sql_query($sql);
		$row = $db->sql_fetchrow($nameresult2);
		// Fix for Undefined index comment
		if (empty($row['comment'])) { $row['comment'] = 'Anonymous'; }
		if (!empty($row) AND ($shout['name'] != "Anonymous") AND ($row['comment'] != "Anonymous")) {
...
function ShoutBoxLayout() {
	...
	// VERSION CHECK
	OpenTable();
	$sql = "SELECT version FROM ".$prefix."_shoutbox_version";
	$result = $db->sql_query($sql);
	$row = $db->sql_fetchrow($result);
	$SHVersion = $row['version'];
	//$VerID = intval($row['id']);
	//$DateCheck = is_numeric($row['datechecked']);
	...
	*/
	if (!empty($SHVersion)) {
	echo "<div align=\"center\" class=\"content\"><strong>"._SB_VERSION."&nbsp;".$SHVersion."</strong></div>";
	}
	/*
	if ($v > $row['version']) {
...
function ShoutBoxThemeing() {
	...
	// Fix for undefined variable $imageList
		if (!empty($image) && $image != '.' && $image != '..' && $image != 'index.html') {
			$imageList .= ''.$image.' ';
		}
	}
...
function themeSubmit($themeColorValues, $totalThemes) {
	global $prefix, $db, $admin_file, $ThemeSel;
...
// End 'Bans' code
// Fix for undefined variable $Submit
...
switch ($Submit) {
	...
	// Fix for undefined variable $timen
	if (!isset($timen)) { $timen = 0; }
	...
	case "ShoutEdit":
	// fix for undefined variable $ShoutError
	// Also changed "none" to language constant
	if (empty($ShoutError)) {
	$ShoutError = '';
	} else {
	$ShoutError = ''._NONE.'';
	...
	case "manageShouts":
	// Fix for undefined variable $page and $pruned
	...
	default:
	// Fix for undefined variable $page & $pruned
	if (empty($page)) { $page = '1'; }
	if (empty($pruned)) { $pruned = '0'; }
    manageShouts($page, $pruned);
	break;
