...
class RNComm_FormBase {
	...
	function display() {
		...
		if ($itemCount > 0) {
			...
			echo '<br /><br /><center>' . _RNC_NO_COMMENTS_FOUND . '</center><br />';
		}

		if ($itemCount > 0 && $this->adminFeatures && $this->selModule != 'forums') {
			echo '<center><input type="submit" name="delete" value="' . _RNC_DELETE .
				'" onclick="document.rncPressed = this.name" /></center>';
		}
		echo '</form><br />';
	}
	...
	function displayMenu() {
		...
		echo '<br /><span class="title">' . _RNC_COMMENTS_USER . '</span>'
			.'<input type="text" name="userName" value="' . htmlspecialchars(str_replace('%', '*', $this->user))
