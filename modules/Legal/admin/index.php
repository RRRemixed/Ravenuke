...
/*
 * Just double-check the admin status of the admin user for this module.
 * First check is for Super Admin status, but if not a Super Admin, next
 * need to check for module admin status.
 */
$lgl_isAdmin = false;
$aid = substr($aid, 0, 25);
$row = $db->sql_fetchrow($db->sql_query('SELECT `name`, `radminsuper` FROM `'.$prefix.'_authors` WHERE `aid`=\''.$aid.'\''));
if ($row['radminsuper'] == 1) $lgl_isAdmin = true;
if (!$lgl_isAdmin) {
	$row = $db->sql_fetchrow($db->sql_query('SELECT `title`, `admins` FROM `'.$prefix.'_modules` WHERE `title`=\''.$lgl_modName.'\''));
	$lgl_admins = explode(',', $row['admins']);
	foreach ($lgl_admins as $i) {
		if ($i == $aid) {
			$lgl_isAdmin = true;
			break;
		}
