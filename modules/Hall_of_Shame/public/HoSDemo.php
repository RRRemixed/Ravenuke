...
$pagetitle = " - "._HoS_HALLOFSHAME.": "._HoS_BROWSEDEMOS;
$pperpage = $hos_config['pperpage'];
include("header.php");
$rtitle = _HoS_PUNKSWITHDEMOS;
mainheader(4, _HoS_HALLOFSHAME.": "._HoS_BROWSEDEMOS, _HoS_INDEXMESS1);
$memid = stripslashes($hos_config['mid']);
$memtbl = stripslashes($hos_config['membertable']);
$memname = stripslashes($hos_config['membername']);
$yes = "Yes";

$result = $db->sql_query("SELECT count(*) FROM ".$prefix."_hos_punks WHERE punkdemolabel = '".$yes."'");
...
echo "<br>\n";
OpenTable();
$result = $db->sql_query("SELECT * FROM ".$prefix."_hos_punks WHERE punkdemolabel = '".$yes."' ORDER BY $sort $sortasc LIMIT $offset, $pperpage");
 ...
 echo "<hr>\n";
    echo "<table border='1' cellpadding='2' cellspacing='2' width='99%'>\n";
     ...
     $punkdatebanned = date($hos_config['date_format'], $punkinfo['date_add']);
       ...
       echo "<td align='center'><a href='$punkdemo' target='_blank'>$punkdemolabel</a></td>\n";
  ...
  if ($pages > 1) {
    ...
    echo "<form action='modules.php?name=$module_name&op=HoSDemos' method='post'>\n";
  ...
  if($rid > 0) {
    echo "<center class='title'>"._HoS_NOPUNKSWITHDEMOS."</center>\n";
  } else {
