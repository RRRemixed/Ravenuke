...
$pagetitle = " - "._HoS_HALLOFSHAME.": "._HoS_SEARCHNAMES;
include("header.php");
mainheader(7, _HoS_HALLOFSHAME.": "._HoS_SEARCHNAMES, _HoS_INDEXMESS1);
$memid = stripslashes($hos_config['mid']);
$memtbl = stripslashes($hos_config['membertable']);
$memname = stripslashes($hos_config['membername']);
echo "<br>\n";
OpenTable();
$result = $db->sql_query("SELECT * FROM ".$prefix."_hos_punks WHERE punkname LIKE '%$query%' OR punknotes LIKE '%$query%' ORDER BY punkname DESC LIMIT 0, ".$hos_config['search']);
