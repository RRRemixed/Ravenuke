...
$pagetitle = " - "._HoS_HALLOFSHAME.": "._HoS_SEARCHGUID;
include("header.php");
mainheader(7, _HoS_HALLOFSHAME.": "._HoS_SEARCHGUID, _HoS_INDEXMESS1);
$memid = stripslashes($hos_config['mid']);
$memtbl = stripslashes($hos_config['membertable']);
$memname = stripslashes($hos_config['membername']);
echo "<br>\n";
OpenTable();
$result = $db->sql_query("SELECT * FROM ".$prefix."_hos_punks WHERE punkguid LIKE '%$query%' OR punknotes LIKE '%$query%' ORDER BY punkguid DESC LIMIT 0, ".$hos_config['search']);
  ...
  echo "<hr>\n";
    echo "<table border='1' cellpadding='2' cellspacing='2' width='99%'>\n";
     ...
     while($punkinfo = $db->sql_fetchrow($result)) {
       ...
       $punkguid = stripslashes($punkinfo['punkguid']);
       ...
       echo "<td align='center'>$punkguid</td>\n";
