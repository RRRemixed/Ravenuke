...
if($rid > 0) { $where = " WHERE punkreason='$rid'"; } else { $where = ""; }
...
if($db->sql_numrows($result) > 0) {
  if($rid != "0") {
    list($res_title) = $db->sql_fetchrow($db->sql_query("SELECT title from ".$prefix."_hos_reasons where rid = '$rid'"));
    $res_title = stripslashes($res_title);
    $rtitle = _HoS_BROWSEREASON.": $res_title";
  } else {
  ...
  if ($pages > 1) {
    ...
    echo "<form action='modules.php?name=$module_name&op=HoSReasons' method='post'>\n";
  ...
  if($rid > 0) {
    echo "<center class='title'>"._HoS_NOPUNKSBANNEDFORREASON."</center>\n";
  } else {
