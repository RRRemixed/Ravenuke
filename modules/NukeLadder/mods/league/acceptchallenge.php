...
SET challenged =".$xdb->qstr(leaguemod_gamevs.$challenge['loser'])." 
WHERE name=".$xdb->qstr($challenge['winner'])." 
AND ladder_id=".$xdb->qstr($event['sid']));

modifysql("UPDATE", X1_DB_teamsevents, " 
SET challenged =".$xdb->qstr(leaguemod_gamevs.$challenge['winner'])." 
...
".$xdb->qstr($extra1).",
".$xdb->qstr($extra2).",
".$xdb->qstr($challenge['extra3'].leaguemod_followup.$_POST['comments2'])."
)");

modifysql("DELETE FROM", X1_DB_teamtempchallenges, "WHERE randid = ".$xdb->qstr($challenge['randid']));

$c .= leaguemod_challaccepted; 
?>