...
if (!isset($lgl_lang['LGL_COM_CONTACTMENU'])) {
	...
	if (file_exists($lgl_modPath . 'language/lang-' . $currentlang . '.php')) {
		...
		$lgl_langS = $language;
	} elseif (file_exists($lgl_modPath . 'language/lang-french.php')) { // Default module lang
		include_once $lgl_modPath . 'language/lang-french.php';
	}
}
/**
 * Legal Document
	...
	function dbGetDocument($doc='', $lang='')
	{
		...
		while ($getAttempts < 2) {
			...
			if ($getAttempts == 0) {
				...
				$sql = $sqlBase . ' AND b.`language` = \'french\''
					. ' AND d.`language` = \'french\''
					. ' AND ' . $docSQL;
				++$getAttempts;
			}
