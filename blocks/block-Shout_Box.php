...
global $prefix, $ShoutSubmit, $ShoutComment, $db, $user, $cookie, $currentlang, $shoutuid, $top_content, $mid_content, $bottom_content, $ShoutMarqueewidth, $ShoutMarqueeheight, $currentlang, $user_prefix;

if ($currentlang) {
		include_once('modules/Shout_Box/language/lang-'.$currentlang.'.php');
	} else {
		include_once('modules/Shout_Box/language/lang-english.php');
	}
function ShoutBox($ShoutSubmit, $prefix, $ShoutComment, $db, $user, $cookie, $shoutuid) {

	global $admin, $admin_file, $currentlang, $top_content, $mid_content, $bottom_content, $ShoutMarqueewidth, $ShoutMarqueeheight, $user_prefix;

	$self = ereg_replace("/", "",$_SERVER['PHP_SELF']);
	// removed as now defined in mainfile my montego
	...
	//$username = $cookie[1];
	cookiedecode($user);
	if (count($cookie) < 2) $username = '';
	else $username = $cookie[1];
	...
	if (!validIP($uip)) $uip = 'noip'; // make sure the IP address is valid!
	// fix undefined variable $BannedShouter
	if (!isset($BannedShouter)) { $BannedShouter = ''; }
	...
	if (!empty($shoutuid)) { $username = "$shoutuid"; }
	//shoutuid tests
	...
	if (!empty($themeRow['blockBackgroundImage']) AND file_exists("modules/Shout_Box/images/background/".$themeRow['blockBackgroundImage']."")) {
		$showBackground = 'yes';
	} else {
		$showBackground = 'no';
	}

	if (file_exists("modules/Shout_Box/images/up/".$themeRow['blockArrowColor']."") && !empty($themeRow['blockArrowColor'])) {
		$up_img = "modules/Shout_Box/images/up/$themeRow[blockArrowColor]";
	} else {
		$up_img = "modules/Shout_Box/images/up/Black.gif";
	}
	if (file_exists("modules/Shout_Box/images/down/".$themeRow['blockArrowColor']."") && !empty($themeRow['blockArrowColor'])) {
		$down_img = "modules/Shout_Box/images/down/$themeRow[blockArrowColor]";
	} else {
		$down_img = "modules/Shout_Box/images/down/Black.gif";
	}
	if (file_exists("modules/Shout_Box/images/pause/".$themeRow['blockArrowColor']."") && !empty($themeRow['blockArrowColor'])) {
	...
	$top_content = "";
	// Fix undefined variable $ShoutError
	if (isset($ShoutError)) {
	...
	if ($conf['reversePosts'] == "no") {
		for ($j = 0; $j < $conf['number']; $j++) {
			// Fix for Undefined offset 1 through to 9
			if(!empty($tempContent[$j])) {
	...
	if ($conf['anonymouspost'] == "no" && $username == "Anonymous") {
		...
		// Fix Undefined variable $PreviousUsername
			if (!empty($PreviousUsername)) { $boxtext = $PreviousUsername; } else { $boxtext = ""._NAME.""; }
		...
		// Fix for undefined variable $PreviousComment
		if (!empty($PreviousComment))
		{
			$boxtext = $PreviousComment;
		} else {
			$boxtext = ''._SB_MESSAGE.'';
		}
