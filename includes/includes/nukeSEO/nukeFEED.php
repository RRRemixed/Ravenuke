...
#      header('Status: 404 Not Found');
  ...
  elseif ($fid == 0 and $type == 'OPML')
  {
    ...
    while ($row = $db->sql_fetchrow($result)) {
      ...
      $item->link =$baseURL.'modules.php?name=News&file=article&sid='.$rsid; 
      $item->syndicationURL =$baseURL.'modules.php?name=News&amp;file=article&amp;sid='.$rsid; 
...
#      $cachetime    = $feed['cachetime'];
      ...
      if(!defined('ADMIN_FILE') and $desc > '') echo '<br /><i>'.$desc.'</i>';
...
if ( !function_exists('seoFeedCountChicklet') )
{
  function seoFeedCountChicklet($fBAddress, $seo_config)
  {
    ...
    $chickletHTML = '<a href="http://feeds.feedburner.com/'.$fBAddress.'"><img width="88" height="26" src="http://feeds.feedburner.com/~fc/'.$fBAddress.'?bg='.$seo_config['feedcount_body'].'&amp;fg='.$seo_config['feedcount_text'].'&amp;anim=0" style="border: 0pt none ; valign=top;" alt="" /></a>';
    return $chickletHTML;
  }
}
