...
function mysql_table_exists($table) {
	global $db;
	$exists = $db->sql_query('SELECT 1 FROM `' . $table . '` LIMIT 0');
...
if (!mysql_table_exists(_RWS_WIW_TABLE_HEAP)) {
	...
	/*********************************************************************/
	$result = $db->sql_query('CREATE TABLE ' . _RWS_WIW_TABLE_HEAP . ' (who VARCHAR(20), mn VARCHAR(20)) TYPE=memory');
	if (!$result) {
		$db->sql_query('CREATE TABLE ' . _RWS_WIW_TABLE_HEAP . ' (who VARCHAR(20), mn VARCHAR(20)) TYPE=heap');
	}
}
...
if (is_user($user)) {
	...
	$safe['who'] = mysql_real_escape_string($userInfo[2]);
	...
	$safe['mn'] = mysql_real_escape_string($rwsresult[0]);
