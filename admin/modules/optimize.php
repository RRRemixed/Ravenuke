...
if ($row['radminsuper'] == 1) {
	...
	OpenTable();
	echo '<center><span class="title">' . _OPTIMIZINGDB . ' ' . $dbname . '</span></center><br /><br />'
	...
	if ($db->sql_numrows($result)) {
		while ($row = $db->sql_fetchrow($result)) {
			...
			if ($gain == 0) {
				...
				echo '<tr><td><b>' . $row[0] . '</b></td>' . '<td><b>' . $total . ' Kb' . '</b></td>' . '<td><b>'
					. _OPTIMIZED . '</b></td><td><b>' . $gain . ' Kb</b></td></tr>';
			}
		}
	}
	echo '</table>';
