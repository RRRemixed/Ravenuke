...
function removeComment ($tid, $sid, $ok=0) {
	...
	if($ok) {
		...
		echo '<br /><br />[ <a href="javascript:history.go(-1)">' , _NO , '</a> | <a class="rn_csrf" href="',$admin_file,'.php?op=RemoveComment&amp;tid=',$tid,'&amp;sid=',$sid,'&amp;ok=1">' , _YES , '</a> ]</center>';
		CloseTable();
...
function RemovePollComment ($tid, $pollID, $ok=0) {
	...
	if($ok) {
		...
		OpenTable();
		echo '<center>' , _SURETODELCOMMENTS;
		echo '<br /><br />[ <a href="javascript:history.go(-1)">' , _NO , '</a> | <a class="rn_csrf" href="',$admin_file,'.php?op=RemovePollComment&amp;tid=',$tid,'&amp;pollID=',$pollID,'&amp;ok=1">' , _YES , '</a> ]</center>';
		CloseTable();
		include_once('footer.php');
	}
}
