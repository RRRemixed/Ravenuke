...
if (($row['radminsuper'] == 1) && ($row['name'] == 'God')) {
	switch ($op) {
		...
		case 'deladmin':
			...
			echo '[ <a class="rn_csrf" href="' . $admin_file . '.php?op=deladmin2&amp;del_aid=' . $del_aid . '">' . _YES . '</a> | <a href="' . $admin_file . '.php?op=mod_authors">' . _NO . '</a> ]</center>';
			CloseTable();
			include_once 'footer.php';
			break;
		case 'deladmin2':
...
function displayadmins() {
	...
	echo '</tr><tr><td>&nbsp;</td>'
		...
		. '<tr><td>' . _PASSWORD . '</td>'
		. '<td colspan="3"><input type="password" name="add_pwd" size="12" maxlength="40" /> <font class="tiny">' . _REQUIRED . '</font><input type="hidden" name="op" value="AddAuthor" /></td></tr>'
...
function modifyadmin($chng_aid) {
	...
	GraphicAdmin();
	OpenTable();
	echo '<center><span class="title"><b>' . _AUTHORSADMIN . '</b></span></center>';
	CloseTable();
	echo '<br />';
	OpenTable();
	echo '<center><span class="option"><b>' . _MODIFYINFO . '</b></span></center>';
	$adm_aid = $chng_aid;
	...
	if ($row['name'] != 'God') {
		...
		echo '<td><input type="hidden" name="init_super" value="' . $init_supersel . '" /><input type="checkbox" name="chng_radminsuper" value="' . $chng_radminsuper . '"' . $supersel . ' /> <b>' . _SUPERUSER . '</b></td>';
		echo '</tr><tr><td>&nbsp;</td>';
		echo '<td colspan="3"><span class="tiny"><i>' . _SUPERWARNING . '</i></span></td></tr>';
	...
	echo '<tr><td>' . _PASSWORD . ':</td>'
		...
		. '<td colspan="3"><input type="password" name="chng_pwd2" size="12" maxlength="40" /> <font class="tiny">' . _FORCHANGES . '</font><input type="hidden" name="adm_aid" value="' . $adm_aid . '" /><input type="hidden" name="op" value="UpdateAuthor" /></td></tr>'
...
function updateadmin() {
	...
	if (!empty($chng_pwd2)) {
		if ($chng_pwd != $chng_pwd2) {
			...
			GraphicAdmin();
			OpenTable();
			echo _PASSWDNOMATCH . '<br /><br /><center>' . _GOBACK . '</center>';
			CloseTable();
			include_once 'footer.php';
			exit;
		}
