...
function home(){
	global $db, $maintablewidth, $maintablealign;
			IF (!isset($_GET['order']) && !isset($_POST['keywords']))$_GET['order'] = 'Open';
				...
				IF (isset($_POST['ticket'])){
					FOREACH ($_POST['ticket'] AS $ticketid){
						if($_POST[status] == 'Delete'){
							...
							if(!$db->sql_query($query)) $iserror = 1;
							$query = "DELETE FROM nuke_hosting_tickets_tickets WHERE tickets_child = '".$ticketid."'";
							if(!$db->sql_query($query)) $iserror = 1;
							if($iserror == 1) $msg = 'Ticket Deleted';
							else$msg = 'This could not be done at this time';
						}else{
							$query = "UPDATE nuke_hosting_tickets_tickets
									SET tickets_status = '".$_POST['status']."'
									WHERE tickets_id = '".$ticketid."'";
	
							IF ($db->sql_query($query)) $msg = 'Ticket '.$_POST['status'];
							ELSE $msg = 'This could not be done at this time';
						}
	                }
	            }ELSE $msg = 'Please select a Ticket.';
?>
				<table width="<?php echo $maintablewidth ?>" cellspacing="1" cellpadding="1" border="1"  align="<?php echo $maintablealign ?>">
				  <tr bgcolor="#AACCEE">
...
?>
			...
			<table width="<?php echo $maintablewidth ?>" cellspacing="1" cellpadding="1"  align="<?php echo $maintablealign ?>">
			  <tr bgcolor="<?php echo $background ?>">
				<td><p>Recent Tickets: <?php echo $totaltickets ?> - Click on the Ticket ID to read the ticket.</p></td>
			  </tr>
			</table>
				...
				IF ($_POST['message'] == '')
					...
					ELSE
						...
						$query = "INSERT INTO nuke_hosting_tickets_tickets
								(tickets_uid,tickets_subject,tickets_timestamp,tickets_urgency,tickets_category,tickets_admin,tickets_child,tickets_question)
								 VALUES('".$_POST[uid]."','".$_POST['postsubject']."','".$date[mon]."/".$date[mday]."/".$date[year]." - ".$date[hours].":".$date[minutes].":".$date[seconds]."',
								 '".$urgency['0']."','".$category['0']."','".$aid."','".$_GET['ticketid']."','".$_POST['message']."')";
...
<?php
			IF (!isset($error)){
				$query = "	INSERT INTO nuke_hosting_tickets_categories (tickets_categories_name) VALUES ('".$_POST['firstname']."')";
...
<?php
			if(!isset($error)){
				$query = "	INSERT INTO nuke_hosting_tickets_status (tickets_status_name) Values('".$_POST['firstname']."')";
				$result = $db->sql_query($query);
?>
					<tr><td colspan="2"><p><b>Everythings OK Status added.</b></p></td></tr>
<?php
			}
?>
				</table><br>
